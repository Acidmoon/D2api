...
describe("admin settings auth source defaults helpers", () => {
  it("builds auth source defaults state from flat settings fields", () => {
    ...
    expect(state.email).toEqual({
      balance: 9.5,
      concurrency: 3,
      subscriptions: [{ value: 1, validity_days: 30 }],
    ...
    expect(state.linuxdo).toEqual({
      balance: 6,
      concurrency: 8,
      subscriptions: [{ value: 2, validity_days: 60 }],
  ...
  it("appends auth source defaults back onto update payload", () => {
    ...
    appendAuthSourceDefaultsToUpdateRequest(payload, {
      email: {
        balance: 1.25,
        concurrency: 2,
        subscriptions: [{ value: 3, validity_days: 7 }],
        grant_on_signup: true,
        grant_on_first_bind: false,
        platform_quotas: {},
      },
      ...
      oidc: {
        balance: 4,
        concurrency: 9,
        subscriptions: [{ value: 9, validity_days: 90 }],
        grant_on_signup: true,
        grant_on_first_bind: true,
        platform_quotas: {},
      },
