...
<template>
  <div v-if="allEndpoints.length > 0" class="flex flex-wrap gap-2">
    <div
      ...
      :key="index"
      class="swiss-panel flex items-center gap-1.5 px-2.5 py-1.5 text-xs transition-colors hover:border-[color:var(--nm-ink)]"
    >
      <span class="font-medium text-gray-600 dark:text-gray-300">{{ item.name }}</span>
      <span
        v-if="item.isDefault"
        class="badge badge-primary px-1 py-px text-[10px] leading-tight"
      ...
      <div class="group/endpoint relative flex items-center gap-1.5">
        <div
          class="swiss-panel pointer-events-none absolute bottom-full left-1/2 z-20 mb-2 w-max max-w-[24rem] -translate-x-1/2 translate-y-1 px-3 py-2.5 text-left opacity-0 transition-all duration-150 group-hover/endpoint:translate-y-0 group-hover/endpoint:opacity-100 group-focus-within/endpoint:translate-y-0 group-focus-within/endpoint:opacity-100"
        >
          <p
          ...
          <p
            class="flex items-center gap-1.5 text-[11px] leading-4"
            style="color: var(--nm-accent-text)"
            :class="item.description ? 'mt-1.5' : ''"
          >
            <span class="h-1.5 w-1.5" style="background: var(--nm-accent); border-radius: var(--nm-radius-sm)"></span>
        ...
        <a
          ...
          class="rounded p-0.5 text-gray-400 transition-colors hover:text-[color:var(--nm-warning-text)] dark:text-gray-500"
