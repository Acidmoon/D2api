<template>
  <BaseDialog :show="show" :title="t('admin.users.groupConfig')" width="wide" @close="$emit('close')">
    <div v-if="user" class="space-y-6">
      <!-- 用户信息头部 -->
      <div class="flex items-center gap-4 rounded-2xl p-5" style="background: var(--nm-surface-soft); box-shadow: var(--nm-shadow-inset)">
        <div class="flex h-14 w-14 items-center justify-center rounded-full" style="background: var(--nm-surface); box-shadow: var(--nm-shadow-raised-sm)">
          <span class="text-2xl font-semibold" style="color: var(--nm-accent-text)">{{ user.email.charAt(0).toUpperCase() }}</span>
        </div>
        <div class="flex-1">
...
const load = async () => {
  ...
  try {
    ...
    // All active route groups are eligible for user authorization.
