<template>
  ...
  <div>
    <!-- Toolbar -->
    <div class="flex flex-wrap items-center justify-between gap-3 border-b border-border/60 px-4 py-3 sm:px-6">
      ...
      <table class="w-full min-w-max divide-y divide-border">
        <thead class="bg-muted/50">
          <tr>
            ...
            <th
              ...
              :class="sortBy === col.key ? 'text-brand' : 'text-muted-foreground'"
        ...
        <tbody class="divide-y divide-border">
          ...
          <tr
            ...
            v-else
            :key="item.user_id"
            class="cursor-pointer transition-colors hover:bg-secondary/50"
          ...
          >
            ...
            <td class="whitespace-nowrap px-4 py-3 text-right text-sm tabular-nums text-gray-500 dark:text-muted-foreground">{{ item.requests.toLocaleString() }}</td>
            <td class="whitespace-nowrap px-4 py-3 text-right text-sm tabular-nums text-gray-500 dark:text-muted-foreground">{{ fmtTokens(item.input_tokens) }}</td>
            <td class="whitespace-nowrap px-4 py-3 text-right text-sm tabular-nums text-gray-500 dark:text-muted-foreground">{{ fmtTokens(item.output_tokens) }}</td>
            <td class="whitespace-nowrap px-4 py-3 text-right text-sm tabular-nums text-gray-500 dark:text-muted-foreground">{{ fmtTokens(item.cache_tokens) }}</td>
            ...
            <td class="whitespace-nowrap px-4 py-3 text-right text-sm font-medium tabular-nums text-semantic-success">${{ fmtCost(item.actual_cost) }}</td>
          </tr>
        </tbody>
      </table>
    </div>
