<template>
  <div :class="flat ? '' : 'card overflow-hidden'">
    ...
    >
      ...
      <button
        type="button"
        class="inline-flex items-center gap-1 rounded-full px-2 py-1 text-xs font-medium text-brand transition-colors hover:bg-brand/10 disabled:cursor-not-allowed disabled:opacity-50"
      ...
      >
        <template #cell-user="{ row }">
          <div class="text-sm">
            <button
              v-if="row.user?.email"
              class="font-medium text-brand underline decoration-dashed underline-offset-2 transition-colors hover:opacity-80"
        ...
        <template #cell-tokens="{ row }">
          ...
          <div v-else class="flex items-center gap-1.5">
            ...
            >
              <div class="flex h-4 w-4 cursor-help items-center justify-center rounded-full bg-secondary transition-colors group-hover:bg-brand/10">
                <Icon name="infoCircle" size="xs" class="text-muted-foreground group-hover:text-brand" />
              </div>
            </div>
          </div>
        </template>
        ...
        <template #cell-latency="{ row }">
          <div class="flex items-stretch gap-2">
            ...
            <div class="grid grid-cols-[max-content_max-content] items-baseline gap-x-2 gap-y-0.5 text-xs">
              <span class="text-muted-foreground/70">{{ t('usage.latencyFirstToken') }}</span>
              ...
              <span class="text-muted-foreground/70">{{ t('usage.latencyDuration') }}</span>
        ...
        <template #cell-request_id="{ row }">
          <div v-if="row.request_id" class="flex max-w-[160px] items-center gap-1.5">
            <span class="truncate font-mono text-xs text-muted-foreground" :title="row.request_id">
              {{ row.request_id }}
            </span>
            <button
              type="button"
        ...
        <div class="space-y-1.5">
          <div>
            ...
            <div v-if="tokenTooltipData && hasImageInputTokens(tokenTooltipData)" class="flex items-center justify-between gap-4">
              <span class="text-muted-foreground">{{ t('usage.imageInputTokens') }}</span>
        ...
        <div class="space-y-1.5">
          ...
          <div class="mb-2 border-b border-border pb-1.5">
            ...
            <div v-if="tooltipData && hasImageInputCost(tooltipData)" class="flex items-center justify-between gap-4">
              <span class="text-muted-foreground">{{ t('usage.imageInputCost') }}</span>
            ...
            <template v-if="tooltipData && !isImageUsage(tooltipData) && (!tooltipData.billing_mode || tooltipData.billing_mode === BILLING_MODE_TOKEN)">
              ...
              <div v-if="tooltipData && hasImageInputTokens(tooltipData)" class="flex items-center justify-between gap-4">
                <span class="text-muted-foreground">{{ t('usage.imageInputTokenPrice') }}</span>
...
const getRequestTypeBadgeClass = (row: AdminUsageLog): string => {
  ...
  if (requestType === 'sync') return 'bg-secondary text-secondary-foreground'
