    ...
    <div class="flex min-h-full items-center justify-center p-4">
      ...
      >
        ...
        <div class="mb-6 text-center">
          <div
            class="mx-auto flex h-12 w-12 items-center justify-center border"
            style="border-color: var(--nm-border); background: var(--nm-surface-soft); border-radius: var(--nm-radius)"
          >
            <svg class="h-6 w-6" style="color: var(--nm-accent-text)" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5">
              <path stroke-linecap="round" stroke-linejoin="round" d="M9 12.75L11.25 15 15 9.75m-3-7.036A11.959 11.959 0 013.598 6 11.99 11.99 0 003 9.749c0 5.592 3.824 10.29 9 11.623 5.176-1.332 9-6.03 9-11.622 0-1.31-.21-2.571-.598-3.751h-.152c-3.196 0-6.1-1.248-8.25-3.285z" />
            </svg>
          </div>
          <h3 class="mt-4 text-xl font-semibold" style="color: var(--nm-ink)">
            {{ t('profile.totp.loginTitle') }}
          </h3>
          <p class="mt-2 text-sm" style="color: var(--nm-ink-muted)">
            {{ t('profile.totp.loginHint') }}
          </p>
          <p v-if="userEmailMasked" class="mt-1 text-sm font-medium" style="color: var(--nm-ink)">
            {{ userEmailMasked }}
          </p>
        </div>
        ...
        <div class="mb-6">
          ...
          <!-- Loading indicator -->
          <div v-if="verifying" class="mt-3 flex items-center justify-center gap-2 text-sm" style="color: var(--nm-ink-muted)">
            <div class="totp-spinner h-4 w-4 animate-spin"></div>
            {{ t('common.verifying') }}
          </div>
        </div>
