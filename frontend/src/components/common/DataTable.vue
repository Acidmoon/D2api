<template>
  <div v-if="!isDesktopViewport" class="space-y-3">
    <template v-if="loading">
      <div v-for="i in 5" :key="i" class="dt-mobile-card p-4">
        <div class="space-y-3">
          <div v-for="column in dataColumns" :key="column.key" class="flex justify-between">
            <div class="dt-skeleton h-4 w-20 animate-pulse"></div>
            <div class="dt-skeleton h-4 w-32 animate-pulse"></div>
          </div>
          <div v-if="hasActionsColumn" class="dt-mobile-actions pt-3">
            <div class="dt-skeleton h-8 w-full animate-pulse"></div>
          </div>
        </div>
      </div>
    </template>
    ...
    <template v-else-if="!data || data.length === 0">
      <div class="dt-empty-card p-12 text-center">
        <slot name="empty">
          <div class="flex flex-col items-center">
            <Icon
              name="inbox"
              size="xl"
              class="dt-empty-icon mb-4 h-12 w-12"
            />
            <p class="dt-empty-title text-lg font-medium">
              {{ t('empty.noData') }}
            </p>
          </div>
        </slot>
    ...
    <template v-else>
      ...
      <div
        ...
        class="dt-mobile-card p-4"
        :class="{ 'dt-row-selected': selectable && isRowSelected(row, index) }"
      >
        <div class="space-y-3">
          ...
          >
            <span class="dt-mobile-label text-xs font-medium uppercase">
              {{ column.label }}
            </span>
            <div class="dt-mobile-value min-w-0 max-w-full text-right text-sm">
      ...
      <thead class="table-header">
        <tr>
          ...
          <th
            ...
            :class="[
              'sticky-header-cell py-3 text-left text-xs font-medium uppercase',
              getAdaptivePaddingClass(),
              { 'dt-sortable-header cursor-pointer': column.sortable },
          ...
          >
            <div :class="['flex items-center space-x-1', getHeaderContentAlignmentClass(column)]">
              ...
              <span v-if="column.sortable" class="dt-sort-icon">
                <svg
                  class="h-2.5 w-2.5"
      ...
      <tbody class="table-body">
        ...
        <tr v-if="loading" v-for="i in 5" :key="i">
          <td v-if="selectable" class="w-11 min-w-11 px-3 py-4">
            <div class="dt-skeleton mx-auto h-4 w-4 animate-pulse"></div>
          </td>
          <td v-for="column in columns" :key="column.key" :class="['whitespace-nowrap py-4', getAdaptivePaddingClass()]">
            <div class="animate-pulse">
              <div class="dt-skeleton h-4 w-3/4"></div>
            </div>
          </td>
        </tr>
        ...
        <tr v-else-if="!data || data.length === 0">
          <td
            ...
            :class="['dt-empty-cell py-12 text-center', getAdaptivePaddingClass()]"
          >
            <slot name="empty">
              <div class="flex flex-col items-center">
                <Icon
        ...
        <template v-else>
          ...
          <tr
            ...
            :ref="item.measure ? rowRef : undefined"
            class="dt-row"
            :class="{ 'dt-row-selected': selectable && isRowSelected(item.row, item.index) }"
            :style="fixedVirtualRowHeight ? { height: `${virtualRowHeight}px` } : undefined"
          >
            <td v-if="selectable" class="dt-select-cell w-11 min-w-11 px-3 py-4 text-center">
              <input
                type="checkbox"
                class="dt-checkbox h-4 w-4"
