<template>
  ...
  <div class="grid grid-cols-2 gap-4 lg:grid-cols-4">
    ...
    <div v-if="!isSimple" class="stat-card">
      ...
      <div class="min-w-0 flex-1">
        ...
        <p class="stat-value">${{ formatBalance(balance) }}</p>
        <p class="mt-0.5 text-xs" style="color: var(--nm-ink-faint)">{{ t('common.available') }}</p>
      </div>
    </div>
    ...
    <div class="stat-card">
      ...
      <div class="min-w-0 flex-1">
        ...
        <p class="mt-0.5 text-xs" style="color: var(--nm-success-text)">{{ stats?.active_api_keys || 0 }} {{ t('common.active') }}</p>
      </div>
    </div>
    ...
    <div class="stat-card">
      ...
      <div class="min-w-0 flex-1">
        ...
        <p class="mt-0.5 text-xs" style="color: var(--nm-ink-faint)">{{ t('common.total') }}: {{ formatNumber(stats?.total_requests || 0) }}</p>
      </div>
    </div>
    ...
    <div class="stat-card">
      ...
      <div class="min-w-0 flex-1">
        ...
        <p class="stat-value">
          ...
          <span class="text-sm font-normal" style="color: var(--nm-ink-faint)" :title="t('dashboard.standard')"> / ${{ formatCost(stats?.today_cost || 0) }}</span>
        </p>
        <p class="mt-0.5 text-xs" style="color: var(--nm-ink-faint)">
  ...
  <!-- 次要指标：紧凑条（Token / 性能 / 响应） -->
  <div class="metric-strip card">
    <div class="metric-item">
      <Icon name="cube" size="sm" :stroke-width="2" class="metric-ic" />
      <span class="metric-k">{{ t('dashboard.todayTokens') }}</span>
      <span class="metric-v">{{ formatTokens(stats?.today_tokens || 0) }}</span>
    </div>
    <span class="metric-sep" />
    <div class="metric-item">
      <Icon name="database" size="sm" :stroke-width="2" class="metric-ic" />
      <span class="metric-k">{{ t('dashboard.totalTokens') }}</span>
      <span class="metric-v">{{ formatTokens(stats?.total_tokens || 0) }}</span>
    </div>
    <span class="metric-sep" />
    <div class="metric-item">
      <Icon name="bolt" size="sm" :stroke-width="2" class="metric-ic" />
      <span class="metric-k">RPM / TPM</span>
      <span class="metric-v">{{ formatTokens(stats?.rpm || 0) }} / {{ formatTokens(stats?.tpm || 0) }}</span>
    </div>
    <span class="metric-sep" />
    <div class="metric-item">
      <Icon name="clock" size="sm" :stroke-width="2" class="metric-ic" />
      <span class="metric-k">{{ t('dashboard.avgResponse') }}</span>
      <span class="metric-v">{{ formatDuration(stats?.average_duration_ms || 0) }}</span>
    </div>
  </div>
  ...
  <div v-if="!isSimple && platformCards.length > 0" class="card p-4">
    <div class="mb-3 flex items-center justify-between">
      <h3 class="text-sm font-semibold" style="color: var(--nm-ink)">{{ t('dashboard.platformBreakdown') }}</h3>
    ...
    <div class="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-4">
      <div
        ...
        :key="item.platform"
        class="platform-cell"
        :class="{ 'platform-cell-other': item.isOther }"
      >
        <div class="flex items-center justify-between">
          <span class="text-sm font-semibold" style="color: var(--nm-ink)">
            {{ item.isOther ? t('dashboard.platformOther') : platformLabel(item.platform) }}
          </span>
          <span class="font-mono text-sm" style="color: var(--nm-accent-text)" :title="t('dashboard.actual')">
            ${{ formatCost(item.total_actual_cost) }}
          </span>
        </div>
        <div class="mt-2 space-y-1 text-xs">
          <div class="flex items-center justify-between">
            <span style="color: var(--nm-ink-faint)">{{ t('dashboard.todayCost') }}</span>
            <span class="font-mono" style="color: var(--nm-ink)">${{ formatCost(item.today_actual_cost) }}</span>
          </div>
          <div class="flex items-center justify-between">
            <span style="color: var(--nm-ink-faint)">{{ t('dashboard.requests') }}</span>
            <span class="font-mono" style="color: var(--nm-ink-muted)">
              {{ item.total_requests > 0 ? formatNumber(item.total_requests) : '-' }}
            </span>
          </div>
          <div class="flex items-center justify-between">
            <span style="color: var(--nm-ink-faint)">{{ t('dashboard.tokens') }}</span>
            <span class="font-mono" style="color: var(--nm-ink-muted)">
