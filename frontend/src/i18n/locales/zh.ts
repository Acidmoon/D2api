export default {
  ...
  admin: {
    ...
    groups: {
      ...
      form: {
        ...
        unavailableAlert: '不可用警告',
        unavailableAlertHint: '开启后，当此分组有账号但一次真实请求无法调度任何账号时，系统会使用 SMTP 配置向邮件设置中的收件人发送提醒。',
        exclusiveLabel: '专属分组',
    ...
    subscriptions: {
      ...
      planHint: '订阅现在是用户级费用额度，不再依赖 APC 订阅分组。',
      limitHint: '留空或填 0 表示该周期不限制；请求会优先消耗订阅额度，耗尽后扣普通余额。',
    ...
    settings: {
      ...
      groupUnavailableAlertNotify: {
        ...
        emails: '收件邮箱',
        emailsHint: '只会向未禁用的邮箱发送；具体是否告警由每个分组的“不可用警告”开关控制。',
        addEmail: '添加邮箱',
  ...
  version: {
    ...
    dockerModeHint: '发现新版本后，请在服务器执行：docker compose pull && docker compose up -d',
    dockerUpdateGuideTitle: 'Docker 部署更新指引',
    dockerUpdateGuide: '请先确认 docker-compose.yml 中镜像地址为 ghcr.io/acidmoon/d2api:latest，然后执行：',
