...
const loadCharts = async () => { loadingCharts.value = true; try { const res = await usageAPI.getDashboardTrend({ start_date: startDate.value, end_date: endDate.value, granularity: granularity.value as any }); trendData.value = res.trend || [] } catch (error) { console.error('Failed to load charts:', error) } finally { loadingCharts.value = false } }
...
const refreshAll = () => { loadStats(); loadCharts(); loadPlatformQuotas() }
