        ...
        <template v-else>
          ...
          <template v-else-if="activeTab === 'subscription'">
            ...
            <template v-if="selectedPlan">
              ...
              <button :class="['btn w-full overflow-hidden text-ellipsis py-3 text-base font-medium', paymentButtonClass]" :disabled="!canSubmitSubscription || submitting" @click="confirmSubscribe">
                <span v-if="submitting" class="flex items-center justify-center gap-2">
                  <span class="h-4 w-4 animate-spin rounded-full border-2 border-white border-t-transparent"></span>
                  {{ t('common.processing') }}
                </span>
                  ...
                  <div v-for="sub in activeSubscriptions" :key="sub.id"
                    ...
                    <div class="min-w-0 flex-1">
                      <div class="flex items-center gap-1.5">
                        <span class="truncate text-xs font-semibold text-gray-900 dark:text-white">{{ sub.plan_name || t('payment.activeSubscription') }}</span>
...
// Renewal modal state is kept for legacy deep links, but plan selection is no longer grouped.
...
onMounted(async () => {
  try {
    ...
    // Handle legacy renewal navigation and direct subscription links.
    if (route.query.tab === 'subscription') {
      activeTab.value = 'subscription'
      const groupId = Number(route.query.group)
      const directPlan = Number.isFinite(groupId)
        ? checkout.value.plans.find((plan) => plan.group_id === groupId)
        : null
      if (directPlan) {
        selectedPlan.value = directPlan
      }
    }
