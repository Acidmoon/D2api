<template>
  <AppLayout>
    <div class="space-y-6">
      <!-- In-content page header + Day Switcher -->
      <div class="page-header">
        <div class="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
          ...
          <div class="flex items-center gap-2">
            <div class="flex rounded-full border border-border p-0.5">
              <button
            ...
            <button @click="loadDashboard" :disabled="loading" class="btn btn-ghost" :title="t('common.refresh')" :aria-label="t('common.refresh')">
      ...
      <template v-else-if="stats">
        ...
        <div class="grid grid-cols-1 gap-6 lg:grid-cols-2">
          ...
          <div class="card p-4">
            ...
            <div v-if="!hasTopUsers(stats.top_users)" class="flex h-32 items-center justify-center text-sm text-muted-foreground">{{ t('payment.admin.noData') }}</div>
            <div v-else class="space-y-2">
              <div v-for="[currency, users] in sortedTopUsers(stats.top_users)" :key="currency" class="space-y-2">
                <p class="text-xs font-semibold text-muted-foreground">{{ currency }}</p>
                <div v-for="(user, idx) in users" :key="user.user_id" class="flex items-center justify-between rounded-lg px-3 py-2 hover:bg-secondary/60">
