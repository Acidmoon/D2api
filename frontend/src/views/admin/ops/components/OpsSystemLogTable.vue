...
const levelBadgeClass = (level: string) => {
  ...
  if (v === 'error' || v === 'fatal') return 'bg-[color:var(--nm-danger-soft)] text-semantic-danger'
  if (v === 'warn' || v === 'warning') return 'bg-[color:var(--nm-warning-soft)] text-semantic-warning'
  if (v === 'debug') return 'bg-secondary text-muted-foreground'
  return 'bg-[color:var(--nm-info-soft)] text-semantic-info'
}
...
<template>
  <section class="card p-4">
    <div class="mb-4 flex flex-wrap items-center justify-between gap-3">
      <div>
        <h3 class="text-sm font-semibold text-foreground">{{ t('admin.ops.systemLogs.title') }}</h3>
        <p class="mt-1 text-xs text-muted-foreground">{{ t('admin.ops.systemLogs.description') }}</p>
      </div>
      <div class="flex flex-wrap items-center gap-2 text-xs">
        <span class="badge badge-gray">{{ t('admin.ops.systemLogs.queue') }} {{ health.queue_depth }}/{{ health.queue_capacity }}</span>
        <span class="badge badge-gray">{{ t('admin.ops.systemLogs.written') }} {{ health.written_count }}</span>
        <span class="badge badge-warning">{{ t('admin.ops.systemLogs.dropped') }} {{ health.dropped_count }}</span>
        <span class="badge badge-danger">{{ t('admin.ops.systemLogs.failed') }} {{ health.write_failed_count }}</span>
      </div>
    </div>

    <div class="mb-4 rounded-xl border border-border bg-secondary/60 p-3">
      <div class="mb-2 flex items-center justify-between">
        <div class="text-xs font-semibold text-foreground">{{ t('admin.ops.systemLogs.runtimeConfig') }}</div>
        <span v-if="runtimeLoading" class="text-xs text-muted-foreground">{{ t('common.loading') }}</span>
      </div>
      ...
      <p class="mt-2 text-xs text-muted-foreground">{{ t('admin.ops.systemLogs.persistAccessLogsHint') }}</p>
      <p v-if="health.last_error" class="mt-2 text-xs text-semantic-danger">{{ t('admin.ops.systemLogs.latestWriteError') }} {{ health.last_error }}</p>
    </div>
    ...
    <div class="overflow-hidden rounded-xl border border-border">
      ...
      <div v-else-if="!hasData" class="px-4 py-8 text-center text-sm text-muted-foreground">{{ t('admin.ops.systemLogs.empty') }}</div>
      <div v-else-if="!isDesktopViewport" class="divide-y divide-border">
        <div v-for="row in logs" :key="row.id" class="space-y-1.5 p-3">
          <div class="flex items-center justify-between gap-2">
            ...
            <span class="text-xs text-muted-foreground">{{ formatTime(row.created_at) }}</span>
          </div>
          <div v-if="row.host" class="truncate text-xs text-muted-foreground" :title="row.host">
            {{ row.host }}
          </div>
      ...
      <div v-else class="overflow-auto">
        <table class="min-w-full table-fixed divide-y divide-border">
          <thead class="bg-secondary/60">
            <tr>
              <th class="w-[170px] px-3 py-2 text-left text-[11px] font-semibold text-muted-foreground">{{ t('admin.ops.systemLogs.time') }}</th>
              <th class="w-[160px] px-3 py-2 text-left text-[11px] font-semibold text-muted-foreground">{{ t('admin.ops.systemLogs.host') }}</th>
              <th class="w-[80px] px-3 py-2 text-left text-[11px] font-semibold text-muted-foreground">{{ t('admin.ops.systemLogs.level') }}</th>
              <th class="px-3 py-2 text-left text-[11px] font-semibold text-muted-foreground">{{ t('admin.ops.systemLogs.logDetails') }}</th>
            </tr>
          </thead>
