...
<template>
  <div class="card p-6">
    <div class="mb-4 flex flex-wrap items-start justify-between gap-3 sm:gap-4">
      <div>
        <h3 class="text-sm font-semibold text-foreground">{{ t('admin.ops.alertRules.title') }}</h3>
    ...
    <div v-else-if="sortedRules.length === 0" class="rounded-xl border border-dashed border-border p-8 text-center text-sm text-muted-foreground">
      ...
      <div class="max-h-[520px] overflow-y-auto">
        <div v-if="!isDesktopViewport" class="divide-y divide-border">
          <div v-for="row in sortedRules" :key="row.id" class="space-y-2 p-4">
            <div class="flex items-start justify-between gap-2">
              <div class="min-w-0">
                <div class="text-xs font-semibold text-foreground">{{ row.name }}</div>
                <div v-if="row.description" class="mt-0.5 line-clamp-2 text-[11px] text-muted-foreground">
                  {{ row.description }}
                </div>
              </div>
              <span class="shrink-0 text-xs font-semibold text-foreground">{{ row.severity }}</span>
            </div>
            ...
            <div v-if="row.updated_at" class="text-[10px] text-muted-foreground">
        ...
        <table v-else class="min-w-full divide-y divide-border">
          ...
          <tbody class="divide-y divide-border bg-card">
            <tr v-for="row in sortedRules" :key="row.id" class="hover:bg-secondary/60">
              <td class="px-4 py-3">
                <div class="text-xs font-semibold text-foreground">{{ row.name }}</div>
                <div v-if="row.description" class="mt-0.5 line-clamp-2 text-[11px] text-muted-foreground">
                  {{ row.description }}
              ...
              <td class="whitespace-nowrap px-4 py-3 text-xs font-semibold text-foreground">
                {{ row.severity }}
              </td>
              <td class="whitespace-nowrap px-4 py-3 text-xs text-foreground">
                {{ row.enabled ? t('common.enabled') : t('common.disabled') }}
    ...
    >
      <div class="space-y-4">
        <div v-if="!editorValidation.valid" class="rounded-xl bg-[color:var(--nm-danger-soft)] p-4 text-xs text-semantic-danger">
        ...
        <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
          ...
          <div class="md:col-span-2">
            <label class="input-label">
              ...
              <span v-if="isGroupMetricSelected" class="ml-1 text-semantic-danger">*</span>
            </label>
            <Select
          ...
          <div class="flex items-center justify-between rounded-xl bg-secondary/60 px-4 py-3 md:col-span-2">
            <span class="text-xs font-semibold text-foreground">{{ t('admin.ops.alertRules.form.enabled') }}</span>
            <input v-model="draft!.enabled" type="checkbox" class="h-4 w-4 rounded border-border accent-[color:var(--nm-accent)]" />
          </div>

          <div class="flex items-center justify-between rounded-xl bg-secondary/60 px-4 py-3 md:col-span-2">
            <span class="text-xs font-semibold text-foreground">{{ t('admin.ops.alertRules.form.notifyEmail') }}</span>
            <input v-model="draft!.notify_email" type="checkbox" class="h-4 w-4 rounded border-border accent-[color:var(--nm-accent)]" />
          </div>
        </div>
      </div>
