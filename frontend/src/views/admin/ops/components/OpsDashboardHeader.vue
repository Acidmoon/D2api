...
const healthScoreColor = computed(() => {
  if (isSystemIdle.value) return '#9fb2ad' // linework gray
  const score = healthScoreValue.value
  if (score == null) return '#9fb2ad'
...
const cpuPercentClass = computed(() => {
  ...
  if (v >= 95) return 'text-semantic-danger'
  if (v >= 80) return 'text-semantic-warning'
  return 'text-semantic-success'
})
...
const memPercentClass = computed(() => {
  ...
  if (v == null) return 'text-foreground'
  if (v >= 95) return 'text-semantic-danger'
  if (v >= 85) return 'text-semantic-warning'
  return 'text-semantic-success'
})
...
const dbMiddleClass = computed(() => {
  if (systemMetrics.value?.db_ok === false) return 'text-semantic-danger'
  if (dbUsagePercent.value != null) {
    if (dbUsagePercent.value >= 90) return 'text-semantic-danger'
    if (dbUsagePercent.value >= 70) return 'text-semantic-warning'
    return 'text-semantic-success'
  }
  if (systemMetrics.value?.db_ok === true) return 'text-semantic-success'
...
const redisMiddleClass = computed(() => {
  if (systemMetrics.value?.redis_ok === false) return 'text-semantic-danger'
  if (redisUsagePercent.value != null) {
    if (redisUsagePercent.value >= 90) return 'text-semantic-danger'
    if (redisUsagePercent.value >= 70) return 'text-semantic-warning'
    return 'text-semantic-success'
  }
  if (systemMetrics.value?.redis_ok === true) return 'text-semantic-success'
  return 'text-foreground'
})
...
<template>
  <div :class="[props.fullscreen ? 'p-8' : 'p-5 sm:p-6', 'card flex flex-col gap-4']">
    <!-- Top Toolbar (title lives in the page header; hidden in fullscreen) -->
    <div v-if="!props.fullscreen" class="flex flex-wrap items-center justify-between gap-4 border-b border-border pb-4">
      <div v-if="!props.fullscreen" class="flex items-center gap-3 text-xs text-muted-foreground">
        <span class="flex items-center gap-1.5" :title="props.loading ? t('admin.ops.loadingText') : t('admin.ops.ready')">
          <span class="relative flex h-2 w-2">
            <span class="relative inline-flex h-2 w-2 rounded-full" :class="props.loading ? 'bg-muted-foreground' : 'bg-[color:var(--nm-success)]'"></span>
          </span>
      ...
      <div class="flex flex-wrap items-center gap-3">
        ...
        <div v-if="!props.fullscreen" class="mx-1 hidden h-4 w-[1px] bg-border sm:block"></div>

        <!-- Alert Rules Button (hidden in fullscreen) -->
        <button
          v-if="!props.fullscreen"
          type="button"
          class="swiss-action text-brand"
    ...
    <div v-if="overview" class="grid grid-cols-1 gap-6 lg:grid-cols-12">
      ...
      <div :class="['rounded-2xl bg-secondary/60 lg:col-span-5', props.fullscreen ? 'p-6' : 'p-4']">
        <div class="grid h-full grid-cols-1 gap-6 md:grid-cols-[200px_1fr] md:items-center">
          <!-- 1) Health Score -->
          <div
            class="group relative flex cursor-pointer flex-col items-center justify-center rounded-xl py-2 transition-all hover:bg-card/60 md:border-r md:border-border md:pr-6"
          >
            ...
            >
              <div class="rounded-xl border border-border bg-popover p-4 shadow-xl">
                <h4 class="mb-3 flex items-center gap-2 border-b border-border pb-2 text-sm font-semibold text-foreground">
                  <Icon name="brain" size="sm" class="text-brand" />
                ...
                <div class="space-y-3">
                  <div v-for="(item, idx) in diagnosisReport" :key="idx" class="flex gap-3">
                    <div class="mt-0.5 shrink-0">
                      <svg v-if="item.type === 'critical'" class="h-4 w-4 text-semantic-danger" fill="currentColor" viewBox="0 0 20 20">
                        <path
                          fill-rule="evenodd"
                      ...
                      <svg v-else-if="item.type === 'warning'" class="h-4 w-4 text-semantic-warning" fill="currentColor" viewBox="0 0 20 20">
                        <path
                          fill-rule="evenodd"
                      ...
                      <svg v-else class="h-4 w-4 text-semantic-info" fill="currentColor" viewBox="0 0 20 20">
                        <path
                          fill-rule="evenodd"
                    ...
                    <div class="flex-1">
                      ...
                      <div v-if="item.action" class="mt-1 flex items-center gap-1 text-[11px] text-brand">
            ...
            <div class="relative flex items-center justify-center">
              ...
              <div class="absolute flex flex-col items-center">
                <span :class="[props.fullscreen ? 'text-5xl' : 'text-3xl', 'font-semibold tabular-nums', healthScoreClass]">
                  {{ isSystemIdle ? t('admin.ops.idleStatus') : (overview.health_score ?? '--') }}
                </span>
                <span :class="[props.fullscreen ? 'text-xs' : 'text-[10px]', 'font-semibold uppercase tracking-wider text-muted-foreground']">{{ t('admin.ops.health') }}</span>
              </div>
            </div>
          ...
          <div class="flex h-full flex-col justify-center py-2">
            <div class="mb-3 flex flex-wrap items-center justify-between gap-2">
              <div class="flex items-center gap-2">
                ...
                <h3 class="text-xs font-semibold uppercase tracking-wider text-muted-foreground">{{ t('admin.ops.realtime.title') }}</h3>
              ...
              <div class="flex flex-wrap gap-1">
                <button
                  ...
                  :key="window"
                  type="button"
                  class="rounded-full px-1.5 py-0.5 text-[9px] font-semibold transition-colors sm:px-2 sm:text-[10px]"
            ...
            <div :class="props.fullscreen ? 'space-y-4' : 'space-y-3'">
              <!-- Row 1: Current -->
              <div>
                <div :class="[props.fullscreen ? 'text-xs' : 'text-[10px]', 'font-semibold uppercase text-muted-foreground']">{{ t('admin.ops.current') }}</div>
                <div class="mt-1 flex flex-wrap items-baseline gap-x-4 gap-y-2">
                  <div class="flex items-baseline gap-1.5">
                    <span :class="[props.fullscreen ? 'text-4xl' : 'text-xl sm:text-2xl', 'font-semibold tabular-nums text-foreground']">{{ displayRealTimeQps.toFixed(1) }}</span>
                    <span :class="[props.fullscreen ? 'text-sm' : 'text-xs', 'font-semibold text-muted-foreground']">QPS</span>
                  </div>
                  <div class="flex items-baseline gap-1.5">
                    <span :class="[props.fullscreen ? 'text-4xl' : 'text-xl sm:text-2xl', 'font-semibold tabular-nums text-foreground']">{{ displayRealTimeTps.toFixed(1) }}</span>
                    <span :class="[props.fullscreen ? 'text-sm' : 'text-xs', 'font-semibold text-muted-foreground']">{{ t('admin.ops.tps') }}</span>
                  </div>
                </div>
              </div>
              ...
              <div class="grid grid-cols-2 gap-3">
                <!-- Peak -->
                <div>
                  <div :class="[props.fullscreen ? 'text-xs' : 'text-[10px]', 'font-semibold uppercase text-muted-foreground']">{{ t('admin.ops.peak') }}</div>
                  <div :class="[props.fullscreen ? 'text-base' : 'text-sm', 'mt-1 space-y-0.5 font-medium text-muted-foreground']">
                    <div class="flex items-baseline gap-1.5">
                ...
                <!-- Average -->
                <div>
                  <div :class="[props.fullscreen ? 'text-xs' : 'text-[10px]', 'font-semibold uppercase text-muted-foreground']">{{ t('admin.ops.average') }}</div>
                  <div :class="[props.fullscreen ? 'text-base' : 'text-sm', 'mt-1 space-y-0.5 font-medium text-muted-foreground']">
                    <div class="flex items-baseline gap-1.5">
      ...
      <div class="grid h-full grid-cols-1 content-center gap-4 sm:grid-cols-2 lg:col-span-7 lg:grid-cols-3">
        <!-- Card 1: Requests -->
        <div class="rounded-2xl bg-secondary/60 p-4" style="order: 1;">
          <div class="flex items-center justify-between">
            <div class="flex items-center gap-1">
              <span class="text-[10px] font-semibold uppercase text-muted-foreground">{{ t('admin.ops.requestsTitle') }}</span>
          ...
          <div class="mt-2 space-y-2 text-xs">
            <div class="flex justify-between">
              ...
              <span class="font-semibold tabular-nums text-foreground">{{ totalRequestsLabel }}</span>
            </div>
            <div class="flex justify-between">
              <span class="text-muted-foreground">{{ t('admin.ops.tokens') }}:</span>
              <span class="font-semibold tabular-nums text-foreground">{{ totalTokensLabel }}</span>
            </div>
            <div class="flex justify-between">
              <span class="text-muted-foreground">{{ t('admin.ops.avgQps') }}:</span>
              <span class="font-semibold tabular-nums text-foreground">{{ qpsAvgLabel }}</span>
            </div>
            <div class="flex justify-between">
              <span class="text-muted-foreground">{{ t('admin.ops.avgTps') }}:</span>
              <span class="font-semibold tabular-nums text-foreground">{{ tpsAvgLabel }}</span>
            </div>
          </div>
        </div>

        <!-- Card 2: SLA -->
        <div class="rounded-2xl bg-secondary/60 p-4" style="order: 2;">
          <div class="flex items-center justify-between">
            <div class="flex items-center gap-2">
              <span class="text-[10px] font-semibold uppercase text-muted-foreground">{{ t('admin.ops.sla') }}</span>
              <HelpTooltip v-if="!props.fullscreen" :content="t('admin.ops.tooltips.sla')" />
              <span class="h-1.5 w-1.5 rounded-full" :class="getSLAThresholdLevel(slaPercent) === 'critical' ? 'bg-[color:var(--nm-danger)]' : getSLAThresholdLevel(slaPercent) === 'warning' ? 'bg-[color:var(--nm-warning)]' : 'bg-[color:var(--nm-success)]'"></span>
            </div>
            <button
              v-if="!props.fullscreen"
              class="text-[10px] font-semibold text-brand hover:underline"
          ...
          <div class="mt-2 text-3xl font-semibold tabular-nums" :class="getThresholdColorClass(getSLAThresholdLevel(slaPercent))">
            {{ slaPercent == null ? '-' : `${slaPercent.toFixed(3)}%` }}
          </div>
          <div class="mt-3 h-2 w-full overflow-hidden rounded-full bg-muted">
            <div class="h-full transition-all" :class="getSLAThresholdLevel(slaPercent) === 'critical' ? 'bg-[color:var(--nm-danger)]' : getSLAThresholdLevel(slaPercent) === 'warning' ? 'bg-[color:var(--nm-warning)]' : 'bg-[color:var(--nm-success)]'" :style="{ width: `${Math.max((slaPercent ?? 0) - 90, 0) * 10}%` }"></div>
          </div>
          <div class="mt-3 text-xs">
            <div class="flex justify-between">
              <span class="text-muted-foreground">{{ t('admin.ops.exceptions') }}:</span>
              <span class="font-semibold tabular-nums text-foreground">{{ formatNumber((overview.request_count_sla ?? 0) - (overview.success_count ?? 0)) }}</span>
            </div>
          </div>
        </div>

        <!-- Card 4: Request Duration -->
        <div class="rounded-2xl bg-secondary/60 p-4" style="order: 4;">
          <div class="flex items-center justify-between">
            <div class="flex items-center gap-1">
              <span class="text-[10px] font-semibold uppercase text-muted-foreground">{{ t('admin.ops.latencyDuration') }}</span>
          ...
          <div class="mt-2 flex items-baseline gap-2">
            <div class="text-3xl font-semibold tabular-nums text-foreground">
              {{ durationP99Ms ?? '-' }}
            </div>
            <span class="text-xs font-semibold text-muted-foreground">ms (P99)</span>
          </div>
          <div class="mt-3 grid grid-cols-1 gap-x-3 gap-y-1 text-xs 2xl:grid-cols-2">
            <div class="flex items-baseline gap-1 whitespace-nowrap">
              ...
              <span class="font-semibold tabular-nums text-foreground">{{ durationP95Ms ?? '-' }}</span>
              <span class="text-muted-foreground">ms</span>
            </div>
            <div class="flex items-baseline gap-1 whitespace-nowrap">
              <span class="text-muted-foreground">P90:</span>
              <span class="font-semibold tabular-nums text-foreground">{{ durationP90Ms ?? '-' }}</span>
              <span class="text-muted-foreground">ms</span>
            </div>
            <div class="flex items-baseline gap-1 whitespace-nowrap">
              <span class="text-muted-foreground">P50:</span>
              <span class="font-semibold tabular-nums text-foreground">{{ durationP50Ms ?? '-' }}</span>
              <span class="text-muted-foreground">ms</span>
            </div>
            <div class="flex items-baseline gap-1 whitespace-nowrap">
              <span class="text-muted-foreground">Avg:</span>
              <span class="font-semibold tabular-nums text-foreground">{{ durationAvgMs ?? '-' }}</span>
              <span class="text-muted-foreground">ms</span>
            </div>
            <div class="flex items-baseline gap-1 whitespace-nowrap">
              <span class="text-muted-foreground">Max:</span>
              <span class="font-semibold tabular-nums text-foreground">{{ durationMaxMs ?? '-' }}</span>
              <span class="text-muted-foreground">ms</span>
            </div>
          </div>
        </div>
        ...
        <!-- Card 5: TTFT -->
        <div class="rounded-2xl bg-secondary/60 p-4" style="order: 5;">
          <div class="flex items-center justify-between">
            <div class="flex items-center gap-1">
              <span class="text-[10px] font-semibold uppercase text-muted-foreground">TTFT</span>
          ...
          <div class="mt-2 flex items-baseline gap-2">
            <div class="text-3xl font-semibold tabular-nums" :class="getThresholdColorClass(getTTFTThresholdLevel(ttftP99Ms))">
              {{ ttftP99Ms ?? '-' }}
            </div>
            <span class="text-xs font-semibold text-muted-foreground">ms (P99)</span>
          </div>
          <div class="mt-3 grid grid-cols-1 gap-x-3 gap-y-1 text-xs 2xl:grid-cols-2">
            <div class="flex items-baseline gap-1 whitespace-nowrap">
              <span class="text-muted-foreground">P95:</span>
              <span class="font-semibold tabular-nums" :class="getThresholdColorClass(getTTFTThresholdLevel(ttftP95Ms))">{{ ttftP95Ms ?? '-' }}</span>
              <span class="text-muted-foreground">ms</span>
            </div>
            <div class="flex items-baseline gap-1 whitespace-nowrap">
              <span class="text-muted-foreground">P90:</span>
              <span class="font-semibold tabular-nums" :class="getThresholdColorClass(getTTFTThresholdLevel(ttftP90Ms))">{{ ttftP90Ms ?? '-' }}</span>
              <span class="text-muted-foreground">ms</span>
            </div>
            <div class="flex items-baseline gap-1 whitespace-nowrap">
              <span class="text-muted-foreground">P50:</span>
              <span class="font-semibold tabular-nums" :class="getThresholdColorClass(getTTFTThresholdLevel(ttftP50Ms))">{{ ttftP50Ms ?? '-' }}</span>
              <span class="text-muted-foreground">ms</span>
            </div>
            <div class="flex items-baseline gap-1 whitespace-nowrap">
              <span class="text-muted-foreground">Avg:</span>
              <span class="font-semibold tabular-nums" :class="getThresholdColorClass(getTTFTThresholdLevel(ttftAvgMs))">{{ ttftAvgMs ?? '-' }}</span>
              <span class="text-muted-foreground">ms</span>
            </div>
            <div class="flex items-baseline gap-1 whitespace-nowrap">
              <span class="text-muted-foreground">Max:</span>
              <span class="font-semibold tabular-nums" :class="getThresholdColorClass(getTTFTThresholdLevel(ttftMaxMs))">{{ ttftMaxMs ?? '-' }}</span>
              <span class="text-muted-foreground">ms</span>
            </div>
          </div>
        </div>
        ...
        <div class="rounded-2xl bg-secondary/60 p-4" style="order: 3;">
          <div class="flex items-center justify-between">
            <div class="flex items-center gap-1">
              <span class="text-[10px] font-semibold uppercase text-muted-foreground">{{ t('admin.ops.requestErrors') }}</span>
              <HelpTooltip v-if="!props.fullscreen" :content="t('admin.ops.tooltips.errors')" />
            </div>
            <button v-if="!props.fullscreen" class="text-[10px] font-semibold text-brand hover:underline" type="button" @click="openErrorDetails('request')">
              {{ t('admin.ops.requestDetails.details') }}
            </button>
          </div>
          <div class="mt-2 text-3xl font-semibold tabular-nums" :class="getThresholdColorClass(getRequestErrorRateThresholdLevel(errorRatePercent))">
            {{ errorRatePercent == null ? '-' : `${errorRatePercent.toFixed(2)}%` }}
          </div>
          <div class="mt-3 space-y-1 text-xs">
            <div class="flex justify-between">
              <span class="text-muted-foreground">{{ t('admin.ops.errorCount') }}:</span>
              <span class="font-semibold tabular-nums text-foreground">{{ formatNumber(overview.error_count_sla ?? 0) }}</span>
            </div>
            <div class="flex justify-between">
              <span class="text-muted-foreground">{{ t('admin.ops.businessLimited') }}:</span>
              <span class="font-semibold tabular-nums text-foreground">{{ formatNumber(overview.business_limited_count ?? 0) }}</span>
            </div>
          </div>
        </div>

        <!-- Card 6: Upstream Errors -->
        <div class="rounded-2xl bg-secondary/60 p-4" style="order: 6;">
          <div class="flex items-center justify-between">
            <div class="flex items-center gap-1">
              <span class="text-[10px] font-semibold uppercase text-muted-foreground">{{ t('admin.ops.upstreamErrors') }}</span>
              <HelpTooltip v-if="!props.fullscreen" :content="t('admin.ops.tooltips.upstreamErrors')" />
            </div>
            <button v-if="!props.fullscreen" class="text-[10px] font-semibold text-brand hover:underline" type="button" @click="openErrorDetails('upstream')">
              {{ t('admin.ops.requestDetails.details') }}
            </button>
          </div>
          <div class="mt-2 text-3xl font-semibold tabular-nums" :class="getThresholdColorClass(getUpstreamErrorRateThresholdLevel(upstreamErrorRatePercent))">
          ...
          <div class="mt-3 space-y-1 text-xs">
            <div class="flex justify-between">
              ...
              <span class="font-semibold tabular-nums text-foreground">{{ formatNumber(overview.upstream_error_count_excl_429_529 ?? 0) }}</span>
            </div>
            <div class="flex justify-between">
              <span class="text-muted-foreground">429/529:</span>
              <span class="font-semibold tabular-nums text-foreground">{{ formatNumber((overview.upstream_429_count ?? 0) + (overview.upstream_529_count ?? 0)) }}</span>
            </div>
          </div>
        </div>
      </div>
    ...
    <div v-if="overview" class="mt-2 border-t border-border pt-4">
      <div class="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-6">
        ...
        <div class="rounded-xl bg-secondary/60 p-3">
          <div class="flex items-center gap-1">
            <div class="text-[10px] font-semibold uppercase tracking-wider text-muted-foreground">CPU</div>
        ...
        <div class="rounded-xl bg-secondary/60 p-3">
          <div class="flex items-center gap-1">
            <div class="text-[10px] font-semibold uppercase tracking-wider text-muted-foreground">{{ t('admin.ops.memory') }}</div>
        ...
        <div class="rounded-xl bg-secondary/60 p-3">
          <div class="flex items-center gap-1">
            <div class="text-[10px] font-semibold uppercase tracking-wider text-muted-foreground">{{ t('admin.ops.db') }}</div>
            <HelpTooltip v-if="!props.fullscreen" :content="t('admin.ops.tooltips.db')" />
          </div>
          <div class="mt-1 text-lg font-semibold tabular-nums" :class="dbMiddleClass">
            {{ dbMiddleLabel }}
          </div>
          <div v-if="!props.fullscreen" class="mt-1 text-[10px] text-muted-foreground">
        ...
        <div class="rounded-xl bg-secondary/60 p-3">
          <div class="flex items-center gap-1">
            <div class="text-[10px] font-semibold uppercase tracking-wider text-muted-foreground">Redis</div>
            <HelpTooltip v-if="!props.fullscreen" :content="t('admin.ops.tooltips.redis')" />
          </div>
          <div class="mt-1 text-lg font-semibold tabular-nums" :class="redisMiddleClass">
            {{ redisMiddleLabel }}
          </div>
          <div v-if="!props.fullscreen" class="mt-1 text-[10px] text-muted-foreground">
        ...
        <div class="rounded-xl bg-secondary/60 p-3">
          <div class="flex items-center gap-1">
            <div class="text-[10px] font-semibold uppercase tracking-wider text-muted-foreground">{{ t('admin.ops.goroutines') }}</div>
            <HelpTooltip v-if="!props.fullscreen" :content="t('admin.ops.tooltips.goroutines')" />
          </div>
          <div class="mt-1 text-lg font-semibold tabular-nums" :class="goroutineStatusClass">
        ...
        <div class="rounded-xl bg-secondary/60 p-3">
          <div class="flex items-center justify-between gap-2">
            <div class="flex items-center gap-1">
              <div class="text-[10px] font-semibold uppercase tracking-wider text-muted-foreground">{{ t('admin.ops.jobs') }}</div>
              <HelpTooltip v-if="!props.fullscreen" :content="t('admin.ops.tooltips.jobs')" />
            </div>
            <button v-if="!props.fullscreen" class="text-[10px] font-semibold text-brand hover:underline" type="button" @click="openJobsDetails">
              {{ t('admin.ops.requestDetails.details') }}
            </button>
          </div>
    ...
    <BaseDialog :show="showJobsDetails" :title="t('admin.ops.jobs')" width="wide" @close="showJobsDetails = false">
      ...
      <div v-else class="space-y-3">
        ...
        >
          ...
          <div class="mt-2 grid grid-cols-1 gap-2 text-xs text-muted-foreground sm:grid-cols-2">
            <div>
          ...
          <div
            v-if="hb.last_error"
            class="mt-3 rounded-lg bg-[color:var(--nm-danger-soft)] p-2 text-xs text-semantic-danger"
          >
            {{ hb.last_error }}
          </div>
        </div>
