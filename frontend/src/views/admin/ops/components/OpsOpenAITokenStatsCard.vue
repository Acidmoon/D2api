...
<template>
  <section class="card p-4 md:p-5">
    ...
    <div v-if="errorMessage" class="mb-4 rounded-lg bg-[color:var(--nm-danger-soft)] px-3 py-2 text-xs text-semantic-danger">
      {{ errorMessage }}
    </div>
            ...
            <div v-for="row in items" :key="row.model" class="space-y-2 p-3">
              ...
              <div class="grid grid-cols-2 gap-x-3 gap-y-1 text-xs">
                <div class="flex items-baseline justify-between gap-2">
                  <span class="text-muted-foreground">{{ t('admin.ops.openaiTokenStats.table.requestCount') }}</span>
                  <span class="text-foreground">{{ formatInt(row.request_count) }}</span>
                </div>
                <div class="flex items-baseline justify-between gap-2">
                  <span class="text-muted-foreground">{{ t('admin.ops.openaiTokenStats.table.avgTokensPerSec') }}</span>
                  <span class="text-foreground">{{ formatRate(row.avg_tokens_per_sec) }}</span>
                </div>
                <div class="flex items-baseline justify-between gap-2">
                  <span class="text-muted-foreground">{{ t('admin.ops.openaiTokenStats.table.avgFirstTokenMs') }}</span>
                  <span class="text-foreground">{{ formatRate(row.avg_first_token_ms) }}</span>
                </div>
                <div class="flex items-baseline justify-between gap-2">
                  <span class="text-muted-foreground">{{ t('admin.ops.openaiTokenStats.table.totalOutputTokens') }}</span>
                  <span class="text-foreground">{{ formatInt(row.total_output_tokens) }}</span>
                </div>
                <div class="flex items-baseline justify-between gap-2">
                  <span class="text-muted-foreground">{{ t('admin.ops.openaiTokenStats.table.avgDurationMs') }}</span>
                  <span class="text-foreground">{{ formatInt(row.avg_duration_ms) }}</span>
                </div>
                <div class="flex items-baseline justify-between gap-2">
                  <span class="text-muted-foreground">{{ t('admin.ops.openaiTokenStats.table.requestsWithFirstToken') }}</span>
          ...
          <table v-else class="min-w-full text-left text-xs md:text-sm">
            ...
            <tbody>
              <tr
                v-for="row in items"
                :key="row.model"
                class="border-b border-border text-foreground last:border-b-0"
              >
