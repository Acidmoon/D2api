...
function severityBadgeClass(severity: string | undefined): string {
  ...
  if (s === 'p0' || s === 'critical') return 'bg-[color:var(--nm-danger-soft)] text-semantic-danger'
  if (s === 'p1' || s === 'warning') return 'bg-[color:var(--nm-warning-soft)] text-semantic-warning'
  if (s === 'p2' || s === 'info') return 'bg-[color:var(--nm-info-soft)] text-semantic-info'
  if (s === 'p3') return 'bg-secondary text-muted-foreground'
  return 'bg-secondary text-muted-foreground'
}

function statusBadgeClass(status: string | undefined): string {
  const s = String(status || '').trim().toLowerCase()
  if (s === 'firing') return 'bg-[color:var(--nm-danger-soft)] text-semantic-danger ring-[color:var(--nm-danger-soft)]'
  if (s === 'resolved') return 'bg-[color:var(--nm-success-soft)] text-semantic-success ring-[color:var(--nm-success-soft)]'
  if (s === 'manual_resolved') return 'bg-secondary text-muted-foreground ring-border'
...
<template>
  <div class="card p-6">
    <div class="mb-4 flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between sm:gap-4">
      <div>
        <h3 class="text-sm font-semibold text-foreground">{{ t('admin.ops.alertEvents.title') }}</h3>
    ...
    <div v-else-if="empty" class="rounded-xl border border-dashed border-border p-8 text-center text-sm text-muted-foreground">
    ...
    <div v-else class="overflow-hidden rounded-xl border border-border">
      <div class="max-h-[600px] overflow-y-auto" @scroll="onScroll">
        <div v-if="!isDesktopViewport" class="divide-y divide-border">
          <div
            v-for="row in events"
            :key="row.id"
            class="cursor-pointer space-y-2 p-4 hover:bg-secondary/60"
          ...
          >
            ...
            <div v-if="row.description" class="line-clamp-2 text-[11px] text-muted-foreground">
              {{ row.description }}
            </div>
            ...
            <div class="text-[11px] text-muted-foreground">{{ formatDimensionsSummary(row) }}</div>
          </div>
        </div>
        <table v-else class="min-w-full divide-y divide-border">
          <thead class="sticky top-0 z-10 bg-secondary/60">
            <tr>
          ...
          <tbody class="divide-y divide-border bg-card">
            <tr
              v-for="row in events"
              :key="row.id"
              class="cursor-pointer hover:bg-secondary/60"
              @click="openDetail(row)"
              :title="row.title || ''"
            >
      ...
      <div v-else class="space-y-5">
        <div class="rounded-xl bg-secondary/60 p-4">
          <div class="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
            <div>
              ...
              <div v-if="selected.description" class="mt-1 whitespace-pre-wrap text-xs text-muted-foreground">
                {{ selected.description }}
              </div>
            </div>

            <div class="flex flex-wrap gap-2">
              <div class="flex items-center gap-2 rounded-lg border border-border bg-card px-2 py-1">
          ...
          <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
            <div class="rounded-xl bg-secondary/60 p-4">
              <div class="text-xs font-semibold uppercase tracking-wider text-muted-foreground">{{ t('admin.ops.alertEvents.detail.firedAt') }}</div>
              <div class="mt-1 text-sm font-medium text-foreground">{{ formatDateTime(selected.fired_at || selected.created_at) }}</div>
            </div>
            <div class="rounded-xl bg-secondary/60 p-4">
              <div class="text-xs font-semibold uppercase tracking-wider text-muted-foreground">{{ t('admin.ops.alertEvents.detail.resolvedAt') }}</div>
              <div class="mt-1 text-sm font-medium text-foreground">{{ selected.resolved_at ? formatDateTime(selected.resolved_at) : '-' }}</div>
            </div>
            <div class="rounded-xl bg-secondary/60 p-4">
              <div class="text-xs font-semibold uppercase tracking-wider text-muted-foreground">{{ t('admin.ops.alertEvents.detail.ruleId') }}</div>
              <div class="mt-1 flex flex-wrap items-center gap-2">
                <div class="font-mono text-sm font-semibold text-foreground">#{{ selected.rule_id }}</div>
                <a
                  class="swiss-action text-[11px]"
            ...
            <div class="rounded-xl bg-secondary/60 p-4">
              <div class="text-xs font-semibold uppercase tracking-wider text-muted-foreground">{{ t('admin.ops.alertEvents.detail.dimensions') }}</div>
        ...
        <div class="rounded-xl border border-border bg-card p-4">
          <div class="mb-3 flex flex-wrap items-center justify-between gap-3">
            <div>
              <div class="text-sm font-semibold text-foreground">{{ t('admin.ops.alertEvents.detail.historyTitle') }}</div>
          ...
          <div v-else class="overflow-hidden rounded-lg border border-border">
            <table class="min-w-full divide-y divide-border">
              <thead class="bg-secondary/60">
                <tr>
                  <th class="px-3 py-2 text-left text-[11px] font-semibold uppercase tracking-wider text-muted-foreground">{{ t('admin.ops.alertEvents.table.time') }}</th>
                  <th class="px-3 py-2 text-left text-[11px] font-semibold uppercase tracking-wider text-muted-foreground">{{ t('admin.ops.alertEvents.table.status') }}</th>
                  <th class="px-3 py-2 text-left text-[11px] font-semibold uppercase tracking-wider text-muted-foreground">{{ t('admin.ops.alertEvents.table.metric') }}</th>
                </tr>
              </thead>
              <tbody class="divide-y divide-border">
                <tr v-for="it in history" :key="it.id" class="hover:bg-secondary/60">
                  <td class="px-3 py-2 text-xs text-muted-foreground">{{ formatDateTime(it.fired_at || it.created_at) }}</td>
