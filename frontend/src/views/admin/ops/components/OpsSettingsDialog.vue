...
<template>
  <BaseDialog :show="show" :title="t('admin.ops.settings.title')" width="extra-wide" @close="emit('close')">
    ...
    <div v-else-if="runtimeSettings && emailConfig && advancedSettings" class="space-y-6">
      <!-- 验证错误 -->
      <div v-if="!validation.valid" class="rounded-lg bg-[color:var(--nm-warning-soft)] p-3 text-xs text-semantic-warning">
          ...
          <div v-if="emailConfig.alert.enabled">
            ...
            <div class="mt-2 flex flex-wrap gap-2">
              <span
                ...
                :key="email"
                class="inline-flex items-center gap-2 rounded-full bg-[color:var(--nm-info-soft)] px-3 py-1 text-xs font-medium text-semantic-info"
              >
                {{ email }}
                <button type="button" class="text-semantic-info hover:opacity-80" @click="removeRecipient('alert', email)">×</button>
              </span>
            </div>
        ...
        <div class="space-y-4">
          ...
          <div v-if="emailConfig.report.enabled">
            ...
            <div class="mt-2 flex flex-wrap gap-2">
              ...
              >
                {{ email }}
                <button type="button" class="text-semantic-info hover:opacity-80" @click="removeRecipient('report', email)">×</button>
              </span>
            </div>
            <p class="mt-2 text-xs text-muted-foreground">
              {{ t('admin.ops.settings.recipientsHint') }}
      ...
      <!-- 高级设置 -->
      <details class="rounded-2xl bg-secondary/60">
