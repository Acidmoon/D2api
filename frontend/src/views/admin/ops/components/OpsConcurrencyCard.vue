...
function getLoadBarClass(loadPct: number): string {
  if (loadPct >= 90) return 'bg-[color:var(--nm-danger)]'
  if (loadPct >= 70) return 'bg-[color:var(--nm-warning)]'
  if (loadPct >= 50) return 'bg-[color:var(--nm-warning)]'
  return 'bg-[color:var(--nm-success)]'
}
...
function getLoadTextClass(loadPct: number): string {
  if (loadPct >= 90) return 'text-semantic-danger'
  if (loadPct >= 70) return 'text-semantic-warning'
  if (loadPct >= 50) return 'text-semantic-warning'
  ...
  <div class="card flex h-full flex-col p-6">
    ...
    <div class="mb-4 flex shrink-0 items-center justify-between gap-3">
      ...
      <div class="flex items-center gap-2">
        <!-- 用户视图切换按钮 -->
        <button
          class="flex items-center justify-center rounded-full px-2 py-1 transition-colors"
          :class="showByUser
    ...
    <!-- 错误提示 -->
    <div v-if="errorMessage" class="mb-3 shrink-0 rounded-xl bg-[color:var(--nm-danger-soft)] p-2.5 text-xs text-semantic-danger">
      {{ errorMessage }}
    </div>
    ...
    <div
      ...
      class="flex flex-1 items-center justify-center rounded-xl border border-dashed border-border text-sm text-muted-foreground"
    >
    ...
    <div v-else class="flex min-h-0 flex-1 flex-col overflow-hidden rounded-xl border border-border">
      <!-- 维度标题栏 -->
      <div class="flex shrink-0 items-center justify-between border-b border-border bg-secondary/60 px-3 py-2">
      ...
      <div v-else-if="displayDimension === 'user'" class="custom-scrollbar max-h-[360px] flex-1 space-y-2 overflow-y-auto p-3">
        <div v-for="row in (displayRows as UserRow[])" :key="row.key" class="rounded-lg bg-secondary/60 p-2.5">
          <!-- 用户信息和并发 -->
          <div class="mb-1.5 flex items-center justify-between gap-2">
            <div class="flex min-w-0 flex-1 items-center gap-1.5">
              <span class="truncate text-[11px] font-semibold text-foreground" :title="row.username || row.user_email">
            ...
            <div class="flex shrink-0 items-center gap-2 text-[10px]">
              <span class="font-mono font-semibold tabular-nums text-foreground"> {{ row.current_in_use }}/{{ row.max_capacity }} </span>
      ...
      <div v-else-if="displayDimension === 'platform' || displayDimension === 'group'" class="custom-scrollbar max-h-[360px] flex-1 space-y-2 overflow-y-auto p-3">
        <div v-for="row in (displayRows as SummaryRow[])" :key="row.key" class="rounded-lg bg-secondary/60 p-3">
          <!-- 标题行 -->
          <div class="mb-2 flex items-center justify-between gap-2">
            <div class="flex items-center gap-2">
              <div class="truncate text-[11px] font-semibold text-foreground" :title="row.name">
                {{ row.name }}
              </div>
            ...
            <div class="flex shrink-0 items-center gap-2 text-[10px]">
              <span class="font-mono font-semibold tabular-nums text-foreground"> {{ row.used_concurrency }}/{{ row.total_concurrency }} </span>
          ...
          <div class="flex flex-wrap items-center gap-x-3 gap-y-1 text-[10px]">
            ...
            <div class="flex items-center gap-1">
              ...
              <span class="text-muted-foreground">
                <span class="font-semibold text-semantic-success">{{ row.available_accounts }}</span
                >/{{ row.total_accounts }}
              </span>
      ...
      <div v-else class="custom-scrollbar max-h-[360px] flex-1 space-y-2 overflow-y-auto p-3">
        <div v-for="row in (displayRows as AccountRow[])" :key="row.key" class="rounded-lg bg-secondary/60 p-2.5">
          <!-- 账号名称和并发 -->
          <div class="mb-1.5 flex items-center justify-between gap-2">
            ...
            <div class="flex shrink-0 items-center gap-2">
              <!-- 并发使用 -->
              <span class="font-mono text-[11px] font-semibold tabular-nums text-foreground"> {{ row.current_in_use }}/{{ row.max_capacity }} </span>
              <!-- 状态徽章 -->
              <span
              ...
              <span
                v-else
                class="inline-flex items-center gap-1 rounded-full bg-secondary px-1.5 py-0.5 text-[10px] font-medium text-muted-foreground"
              >
