...
const kindBadgeClass = (kind: string) => {
  if (kind === 'error') return 'bg-[color:var(--nm-danger-soft)] text-semantic-danger'
  return 'bg-[color:var(--nm-success-soft)] text-semantic-success'
}
</script>
      ...
      <div class="flex h-full min-h-0 flex-col">
        ...
        <div v-else class="flex min-h-0 flex-1 flex-col">
          <div v-if="items.length === 0" class="rounded-xl border border-dashed border-border p-10 text-center">
            <div class="text-sm font-medium text-muted-foreground">{{ t('admin.ops.requestDetails.empty') }}</div>
          ...
          <div v-else class="flex min-h-0 flex-1 flex-col overflow-hidden rounded-xl border border-border">
            <div class="min-h-0 flex-1 overflow-auto">
              <div v-if="!isDesktopViewport" class="divide-y divide-border">
                <div v-for="(row, idx) in items" :key="idx" class="space-y-2 p-4">
                  <div class="flex flex-wrap items-center gap-2">
                    ...
                    <span class="ml-auto text-[11px] text-muted-foreground">{{ formatDateTime(row.created_at) }}</span>
                  </div>
                  <div class="break-all text-xs text-muted-foreground">{{ row.model || '-' }}</div>
                  ...
                  <div v-if="row.request_id" class="flex items-center gap-2">
                    ...
                    <button
                      class="shrink-0 rounded-full bg-secondary px-2 py-1 text-[10px] font-semibold text-muted-foreground transition-colors hover:bg-muted hover:text-foreground"
                  ...
                  <button
                    ...
                    class="w-full rounded-full bg-[color:var(--nm-danger-soft)] px-3 py-1.5 text-xs font-semibold text-semantic-danger transition-[filter] hover:brightness-95"
              ...
              <tbody class="divide-y divide-border bg-card">
                <tr v-for="(row, idx) in items" :key="idx" class="hover:bg-secondary/60">
                  ...
                  <td class="max-w-[240px] truncate px-4 py-3 text-xs text-muted-foreground" :title="row.model || ''">
                    {{ row.model || '-' }}
                  </td>
                  <td class="whitespace-nowrap px-4 py-3 text-xs text-muted-foreground">
                  ...
                  <td class="px-4 py-3">
                    <div v-if="row.request_id" class="flex items-center gap-2">
                      ...
                      <button
                        class="rounded-full bg-secondary px-2 py-1 text-[10px] font-semibold text-muted-foreground transition-colors hover:bg-muted hover:text-foreground"
                        @click="handleCopyRequestId(row.request_id)"
                      >
                        {{ t('admin.ops.requestDetails.copy') }}
                      </button>
                  ...
                  <td class="whitespace-nowrap px-4 py-3 text-right">
                    <button
                      v-if="row.kind === 'error' && row.error_id"
                      class="rounded-full bg-[color:var(--nm-danger-soft)] px-3 py-1.5 text-xs font-semibold text-semantic-danger transition-[filter] hover:brightness-95"
                      @click="openErrorDetail(row.error_id)"
                    >
                      {{ t('admin.ops.requestDetails.viewError') }}
                    </button>
