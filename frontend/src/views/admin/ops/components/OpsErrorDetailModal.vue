    ...
    <div v-else class="space-y-6 p-6">
      ...
      <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <div class="rounded-xl bg-secondary/60 p-4">
          <div class="text-xs font-semibold uppercase tracking-wider text-muted-foreground">{{ t('admin.ops.errorDetail.requestId') }}</div>
        ...
        <div class="rounded-xl bg-secondary/60 p-4">
          <div class="text-xs font-semibold uppercase tracking-wider text-muted-foreground">{{ t('admin.ops.errorDetail.time') }}</div>
        ...
        <div class="rounded-xl bg-secondary/60 p-4">
          <div class="text-xs font-semibold uppercase tracking-wider text-muted-foreground">{{ t('admin.ops.errorDetail.platform') }}</div>
          <div class="mt-1 text-sm font-medium text-foreground">
            {{ detail.platform || '—' }}
          </div>
        </div>
        ...
        <div class="rounded-xl bg-secondary/60 p-4">
          <div class="text-xs font-semibold uppercase tracking-wider text-muted-foreground">{{ t('admin.ops.errorDetail.group') }}</div>
          <div class="mt-1 text-sm font-medium text-foreground">
        ...
        <div class="rounded-xl bg-secondary/60 p-4">
          <div class="text-xs font-semibold uppercase tracking-wider text-muted-foreground">{{ t('admin.ops.errorDetail.model') }}</div>
          <div class="mt-1 text-sm font-medium text-foreground">
            <template v-if="hasModelMapping(detail)">
              <span class="font-mono">{{ detail.requested_model }}</span>
              <span class="mx-1 text-muted-foreground">→</span>
              <span class="font-mono text-brand">{{ detail.upstream_model }}</span>
            </template>
            <template v-else>
              {{ displayModel(detail) || '—' }}
            </template>
        ...
        <div class="rounded-xl bg-secondary/60 p-4">
          <div class="text-xs font-semibold uppercase tracking-wider text-muted-foreground">{{ t('admin.ops.errorDetail.inboundEndpoint') }}</div>
          <div class="mt-1 break-all font-mono text-sm font-medium text-foreground">
        ...
        <div class="rounded-xl bg-secondary/60 p-4">
          <div class="text-xs font-semibold uppercase tracking-wider text-muted-foreground">{{ t('admin.ops.errorDetail.upstreamEndpoint') }}</div>
          <div class="mt-1 break-all font-mono text-sm font-medium text-foreground">
        ...
        <div class="rounded-xl bg-secondary/60 p-4">
          <div class="text-xs font-semibold uppercase tracking-wider text-muted-foreground">{{ t('admin.ops.errorDetail.status') }}</div>
          <div class="mt-1">
            <span :class="['inline-flex items-center rounded-full px-2 py-1 text-xs font-semibold ring-1 ring-inset', statusClass]">
              {{ detail.status_code }}
            </span>
          </div>
        </div>
        ...
        <div class="rounded-xl bg-secondary/60 p-4">
          <div class="text-xs font-semibold uppercase tracking-wider text-muted-foreground">{{ t('admin.ops.errorDetail.upstreamStatus') }}</div>
          <div class="mt-1">
            <span :class="['inline-flex items-center rounded-full px-2 py-1 text-xs font-semibold ring-1 ring-inset', upstreamStatusClass]">
        ...
        <div class="rounded-xl bg-secondary/60 p-4">
          <div class="text-xs font-semibold uppercase tracking-wider text-muted-foreground">{{ t('admin.ops.errorDetail.requestType') }}</div>
          <div class="mt-1 text-sm font-medium text-foreground">
        ...
        <div class="rounded-xl bg-secondary/60 p-4">
          <div class="text-xs font-semibold uppercase tracking-wider text-muted-foreground">{{ t('admin.ops.errorDetail.message') }}</div>
          <div class="mt-1 break-words text-sm font-medium text-foreground" :title="rootCauseMessage">
            {{ rootCauseMessage || '—' }}
          </div>
        </div>

        <div v-if="detail.api_key_prefix" class="rounded-xl bg-secondary/60 p-4">
          <div class="text-xs font-semibold uppercase tracking-wider text-muted-foreground">{{ t('admin.ops.errorDetail.apiKeyPrefix') }}</div>
      ...
      <div v-if="rootCauseMessage" class="rounded-xl bg-[color:var(--nm-warning-soft)] p-6">
        <h3 class="text-sm font-semibold uppercase tracking-wider text-semantic-warning">{{ t('admin.ops.errorDetail.rootCause') }}</h3>
        <div class="mt-3 break-words text-sm font-medium text-foreground">{{ rootCauseMessage }}</div>
      </div>

      <div class="rounded-xl bg-secondary/60 p-6">
        <h3 class="text-sm font-semibold uppercase tracking-wider text-foreground">{{ t('admin.ops.errorDetail.diagnosticPayloads') }}</h3>
        <div v-if="!diagnosticPayloadSections.length" class="mt-4 text-sm text-muted-foreground">{{ t('common.noData') }}</div>
        <div v-else class="mt-4 space-y-4">
          <div v-for="section in diagnosticPayloadSections" :key="section.key">
            <div class="mb-2 text-xs font-semibold uppercase tracking-wider text-muted-foreground">{{ diagnosticPayloadLabel(section.key) }}</div>
            <pre class="max-h-[520px] overflow-auto rounded-xl border border-border bg-card p-4 text-xs text-foreground"><code>{{ prettyJSON(section.value) }}</code></pre>
          </div>
        </div>
      </div>

      <!-- Upstream errors list (only for request errors) -->
      <div v-if="showUpstreamList" class="rounded-xl bg-secondary/60 p-6">
        <div class="flex flex-wrap items-center justify-between gap-2">
          <h3 class="text-sm font-semibold uppercase tracking-wider text-foreground">{{ t('admin.ops.errorDetails.upstreamErrors') }}</h3>
          ...
          >
            <div class="flex flex-wrap items-center justify-between gap-2">
              <div class="text-xs font-semibold text-foreground">
                #{{ idx + 1 }}
                <span v-if="ev.type" class="ml-2 rounded-full bg-secondary px-2 py-0.5 font-mono text-[10px] font-semibold text-foreground">{{ ev.type }}</span>
              </div>
              <div class="flex items-center gap-2">
                ...
                <button
                  type="button"
                  class="inline-flex items-center gap-1.5 rounded-full px-1.5 py-1 text-[10px] font-semibold text-brand hover:bg-secondary disabled:cursor-not-allowed disabled:opacity-60"
            ...
            <pre
              ...
              class="mt-3 max-h-[240px] overflow-auto rounded-xl border border-border bg-secondary/60 p-3 text-xs text-foreground"
...
function statusBadgeClass(code: number): string {
  if (code >= 500) return 'bg-[color:var(--nm-danger-soft)] text-semantic-danger ring-[color:var(--nm-danger-soft)]'
  if (code === 429) return 'bg-[color:var(--nm-warning-soft)] text-semantic-warning ring-[color:var(--nm-warning-soft)]'
  if (code >= 400) return 'bg-[color:var(--nm-warning-soft)] text-semantic-warning ring-[color:var(--nm-warning-soft)]'
