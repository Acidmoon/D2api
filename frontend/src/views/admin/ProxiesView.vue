  ...
  <AppLayout>
      ...
      <template #filters>
        <div>
          ...
          <div class="page-header">
            <h1 class="page-title">{{ t('admin.proxies.title') }}</h1>
            <p class="page-description">{{ t('admin.proxies.description') }}</p>
          </div>
      ...
      <template #table>
        ...
        >
          <template #header-select>
            <input
              type="checkbox"
              class="h-4 w-4 cursor-pointer rounded border-input text-brand focus:ring-ring"
          ...
          <template #cell-address="{ row }">
            <div class="flex items-center gap-1.5">
              ...
              <div class="relative">
                <button
                  type="button"
                  class="rounded p-0.5 text-muted-foreground transition-colors hover:text-brand"
                ...
                <div
                  ...
                  class="dropdown left-0 top-full mt-1 w-auto min-w-[180px]"
                >
                  <button
          ...
          <template #cell-auth="{ row }">
            <div v-if="row.username || row.password" class="flex items-center gap-1.5">
              ...
              <button
                v-if="row.password"
                type="button"
                class="ml-1 rounded p-0.5 text-muted-foreground transition-colors hover:text-foreground"
          ...
          <template #cell-account_count="{ row, value }">
            <button
              v-if="(value || 0) > 0"
              type="button"
              class="inline-flex items-center rounded-full bg-secondary px-2 py-0.5 text-xs font-medium text-brand transition-colors hover:bg-secondary/75"
          ...
          <template #cell-created_at="{ row }">
            <span class="text-xs text-muted-foreground">{{ formatDateTime(row.created_at) }}</span>
          </template>
          ...
          <template #cell-actions="{ row }">
            <div class="flex items-center gap-3 whitespace-nowrap text-sm">
              ...
              <button
                ...
                class="font-medium text-brand transition-opacity hover:opacity-80 hover:underline disabled:cursor-not-allowed disabled:opacity-50 disabled:hover:no-underline"
              >
                {{ t('admin.proxies.testConnection') }}
              </button>
              <!-- Quality Check -->
              ...
              <button
                ...
                class="font-medium text-semantic-danger transition-opacity hover:opacity-80 hover:underline"
              >
                {{ t('common.delete') }}
              </button>
            </div>
    ...
    >
      ...
      <div v-else class="space-y-5">
        ...
        <!-- Parse Result -->
        <div v-if="batchParseResult.total > 0" class="rounded-lg bg-secondary/60 p-4">
            ...
            <div v-if="batchParseResult.invalid > 0" class="flex items-center gap-1.5">
              ...
              />
              <span class="text-semantic-warning">
          ...
          <table class="min-w-full divide-y divide-border text-sm">
            ...
            <tbody class="divide-y divide-border">
              <tr v-for="item in qualityReport.items" :key="item.target">
                ...
                <td class="whitespace-nowrap px-3 py-2 text-muted-foreground">{{ item.http_status ?? '-' }}</td>
