  ...
  <AppLayout>
    <div class="space-y-6">
      ...
      <template v-else>
        ...
        <div class="grid grid-cols-1 gap-3 md:grid-cols-2 xl:grid-cols-4">
          <div
            ...
            :key="item.key"
            class="rounded-lg border border-border/60 bg-card px-4 py-3 shadow-sm"
          >
        ...
        >
          <div data-test="pre-block-sync-card" class="card">
            <div class="flex flex-col gap-4 border-b border-border/60 px-6 py-4 lg:flex-row lg:items-center lg:justify-between">
              <div>
              ...
              <span class="inline-flex w-fit items-center rounded-full bg-muted px-2.5 py-1 text-xs font-medium text-muted-foreground">
          ...
          <div data-test="pre-block-api-key-load-card" class="card">
            ...
            <div class="p-6">
              ...
              <p v-else class="rounded-lg bg-muted/50 p-4 text-sm text-muted-foreground">
        ...
        <div class="card">
          <div class="flex flex-col gap-4 border-b border-border/60 px-6 py-4">
            ...
            <div class="flex flex-col gap-2 rounded-lg border border-border/60 bg-muted/50 px-3 py-2 sm:flex-row sm:items-center sm:justify-between">
              <div class="flex min-w-0 items-center gap-2 text-sm text-foreground">
                <Icon name="filter" size="sm" class="flex-shrink-0 text-muted-foreground/80" />
              ...
              <div v-if="modelFilterPreviewModels.length > 0" class="flex flex-wrap gap-1.5">
                <span
                  ...
                  :key="model"
                  class="inline-flex max-w-[180px] items-center truncate rounded-md bg-card px-2 py-1 font-mono text-xs text-muted-foreground shadow-sm"
                >
                  {{ model }}
                </span>
                <span v-if="hiddenModelFilterModelCount > 0" class="inline-flex rounded-md bg-card px-2 py-1 text-xs text-muted-foreground shadow-sm">
          ...
          <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-border">
              ...
              <tbody class="divide-y divide-border/60 bg-card">
                ...
                <template v-else>
                  <tr v-for="row in logs" :key="row.id" class="hover:bg-secondary/60">
                    ...
                    <td class="whitespace-nowrap px-5 py-4 text-sm text-foreground">
                      ...
                      <div v-if="row.user_id" class="text-xs text-muted-foreground/80">UID {{ row.user_id }}</div>
                    </td>
                    <td class="whitespace-nowrap px-5 py-4 text-sm text-foreground">{{ row.api_key_name || '-' }}</td>
                    <td class="whitespace-nowrap px-5 py-4 text-sm text-foreground">
                      <div>{{ row.endpoint || '-' }}</div>
                      <div class="text-xs text-muted-foreground/80">{{ row.provider || '-' }} / {{ row.model || '-' }}</div>
                    </td>
                    ...
                    <td class="whitespace-nowrap px-5 py-4 text-sm text-foreground">
                      ...
                      <div class="text-xs text-muted-foreground/80">{{ percent(row.highest_score) }}</div>
                    ...
                    <td class="whitespace-nowrap px-5 py-4 text-sm text-foreground">
                      ...
                      <div v-if="row.queue_delay_ms !== null && row.queue_delay_ms !== undefined" class="text-xs text-muted-foreground/80">
                    ...
                    <td class="w-[320px] max-w-sm px-5 py-4 text-sm text-foreground">
                      <button
                        type="button"
                        class="group flex w-full min-w-0 items-center gap-2 rounded-lg px-2 py-1.5 text-left transition-colors hover:bg-secondary"
                        :title="inputSummaryText(row)"
                        @click="openInputDetail(row)"
                      >
                        <span class="min-w-0 flex-1 truncate">{{ inputSummaryText(row) }}</span>
                        <Icon name="eye" size="xs" class="flex-shrink-0 text-muted-foreground/60 transition-colors group-hover:text-brand" />
                      </button>
                    </td>
                  </tr>
                </template>
      ...
      <BaseDialog :show="settingsOpen" :title="t('admin.riskControl.settingsTitle')" width="extra-wide" @close="settingsOpen = false">
        <div class="space-y-6">
          <div class="flex gap-2 overflow-x-auto border-b border-border/60 pb-3">
            <button
              v-for="tab in settingsTabs"
              :key="tab.id"
              type="button"
              class="inline-flex whitespace-nowrap rounded-lg px-3 py-2 text-sm font-medium transition-colors"
              :class="activeSettingsTab === tab.id ? 'bg-brand/10 text-brand' : 'text-muted-foreground hover:bg-secondary hover:text-foreground'"
            ...
            <div class="grid grid-cols-1 gap-5 lg:grid-cols-2">
              ...
              <div>
                ...
                <div class="relative">
                  ...
                  <span class="pointer-events-none absolute right-3 top-1/2 -translate-y-1/2 text-muted-foreground/80">%</span>
                </div>
              </div>
            </div>

            <div class="overflow-hidden rounded-xl border border-border/60 bg-card shadow-sm">
              <div class="flex flex-col gap-4 border-b border-border/60 bg-muted/50 px-4 py-4 lg:flex-row lg:items-center lg:justify-between">
                <div class="flex items-start gap-3">
                  <span class="mt-0.5 flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-lg bg-brand/10 text-brand">
              ...
              <div class="grid grid-cols-1 gap-4 p-4 xl:grid-cols-[minmax(0,1fr)_minmax(360px,440px)]">
                <div class="space-y-3">
                  <div class="flex flex-col gap-2 rounded-lg border border-border/60 bg-muted/50 p-2 sm:flex-row sm:items-center sm:justify-between">
                    ...
                    <div class="inline-flex rounded-lg bg-card p-1 shadow-sm">
                      <button
                        type="button"
                        class="rounded-md px-3 py-1.5 text-xs font-medium transition-colors"
                        :class="configForm.api_keys_mode === 'append' ? 'bg-brand/10 text-brand' : 'text-muted-foreground hover:bg-secondary'"
                      ...
                      <button
                        type="button"
                        class="rounded-md px-3 py-1.5 text-xs font-medium transition-colors"
                        :class="configForm.api_keys_mode === 'replace' ? 'bg-amber-500 text-white shadow-sm' : 'text-muted-foreground hover:bg-secondary'"
                        :disabled="configForm.clear_api_key"
                  ...
                  <div class="rounded-lg border border-border/60 bg-muted/50 p-3" @paste="handleModerationImagePaste">
                    <div class="mb-3 flex items-center justify-between gap-3">
                      <div>
                      ...
                      <button
                        ...
                        type="button"
                        class="inline-flex items-center gap-1 rounded-md px-2 py-1 text-xs font-medium text-muted-foreground hover:bg-card hover:text-muted-foreground hover:bg-secondary hover:text-foreground"
                    ...
                    ></textarea>
                    <div
                      class="mt-3 rounded-lg border border-dashed border-border bg-card p-3"
                      @dragover.prevent
                      @drop.prevent="handleModerationImageDrop"
                    >
                      <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                        <div class="flex items-start gap-2">
                          <Icon name="upload" size="md" class="mt-0.5 text-muted-foreground/80" />
                          <div>
                      ...
                      <div v-if="moderationTestImages.length > 0" class="mt-3 grid grid-cols-2 gap-2 sm:grid-cols-4">
                        <div
                          ...
                          class="group relative aspect-square overflow-hidden rounded-lg border border-border/60 bg-muted"
                        >
                ...
                <div class="rounded-lg border border-border/60 bg-muted/50 p-3">
                  <div class="mb-3 flex items-start justify-between gap-3">
                    ...
                    <span class="inline-flex shrink-0 items-center whitespace-nowrap rounded-full bg-card px-2 py-0.5 text-[11px] font-medium leading-5 text-muted-foreground shadow-sm">
                      {{ t('admin.riskControl.apiKeyRows', { count: apiKeyRows.length }) }}
                    </span>
                  </div>

                  <div v-if="apiKeyRows.length === 0" class="flex min-h-32 flex-col items-center justify-center rounded-lg border border-dashed border-border bg-card px-4 py-6 text-center">
                    <Icon name="infoCircle" size="lg" class="text-muted-foreground/60" />
                  ...
                  <div v-else class="space-y-2">
                    <div class="space-y-2" :class="apiKeyRowsExpanded ? 'max-h-72 overflow-y-auto pr-1' : ''">
                      <div
                        ...
                        class="rounded-lg border bg-card p-2.5 shadow-sm"
                        :class="isStoredApiKeyPendingDelete(row) ? 'border-amber-200 opacity-70 dark:border-amber-800/60' : 'border-border/60'"
                      >
                        <div class="flex items-start justify-between gap-2">
                          <div class="min-w-0">
                            <div class="flex min-w-0 flex-wrap items-center gap-2">
                              ...
                              <span
                                ...
                                :class="row.configured ? 'bg-brand/10 text-brand' : 'bg-purple-50 text-purple-700 dark:bg-purple-900/30 dark:text-purple-300'"
                              >
                          ...
                          <div class="flex flex-shrink-0 items-center gap-1.5">
                            ...
                            <button
                              ...
                              type="button"
                              class="inline-flex h-7 w-7 items-center justify-center rounded-md text-muted-foreground/80 transition-colors hover:bg-secondary hover:text-foreground"
                    ...
                    <div v-if="canToggleApiKeyRows" class="flex items-center justify-between gap-3 rounded-lg border border-dashed border-border bg-card px-3 py-2 text-xs text-muted-foreground">
                      ...
                      <button
                        type="button"
                        class="inline-flex shrink-0 items-center gap-1 rounded-md px-2 py-1 font-medium text-brand transition-colors hover:bg-brand/10"
                  ...
                  <div v-if="moderationTestResult" class="mt-4 rounded-lg border border-border/60 bg-card p-3">
                    ...
                    <div class="mt-3 max-h-52 space-y-2 overflow-y-auto pr-1">
                      <div v-for="score in moderationScoreRows" :key="score.category">
                        <div class="mb-1 flex items-center justify-between gap-3 text-xs">
                          <span class="truncate text-foreground/80">{{ score.category }}</span>
          ...
          <div v-else-if="activeSettingsTab === 'scope'" class="space-y-5">
            <div class="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
              ...
              <div class="inline-flex rounded-lg bg-muted p-1">
                <button
                  ...
                  :class="configForm.all_groups ? 'bg-card text-foreground shadow-sm dark:text-white' : 'text-muted-foreground'"
            ...
            <div v-if="!configForm.all_groups" class="space-y-4">
              <div class="relative">
                <Icon name="search" size="sm" class="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-muted-foreground/80" />
              ...
              <div class="grid max-h-[420px] grid-cols-1 gap-3 overflow-y-auto pr-1 md:grid-cols-2 xl:grid-cols-3">
                <button
                  ...
                  :class="isGroupSelected(group.id) ? 'border-brand bg-brand/10' : 'border-border/60 hover:bg-secondary/60'"
                ...
                >
                  ...
                  <span
                    ...
                    :class="isGroupSelected(group.id) ? 'border-primary bg-primary text-primary-foreground' : 'border-border text-transparent'"
                  >
            ...
            <div class="space-y-4 rounded-lg border border-border/60 p-4">
              <div class="flex flex-col gap-2 lg:flex-row lg:items-center lg:justify-between">
                ...
                <span class="inline-flex w-fit rounded-md bg-muted px-2.5 py-1 text-xs font-medium text-muted-foreground">
                  {{ modelFilterSummary }}
                </span>
              </div>
          ...
          <div v-else-if="activeSettingsTab === 'runtime'" class="grid grid-cols-1 gap-5 lg:grid-cols-2">
            ...
            <div class="flex items-center justify-between rounded-lg border border-border/60 p-4 lg:col-span-2">
              <div>
            ...
            <div class="space-y-4 rounded-lg border border-border/60 p-4 lg:col-span-2">
            ...
            <div class="grid grid-cols-1 gap-5 lg:grid-cols-2">
              ...
              <div class="flex items-center justify-between rounded-lg border border-border/60 p-4 lg:col-span-2">
                <div>
                  ...
                  <p class="mt-1 text-xs text-muted-foreground">{{ t('admin.riskControl.cyberPolicyExcludeBanHint') }}</p>
                </div>
          ...
          <div v-else-if="activeSettingsTab === 'riskThresholds'" class="space-y-5">
            ...
            <div class="grid grid-cols-1 gap-3 md:grid-cols-2 xl:grid-cols-3">
              <div
                ...
                :key="row.category"
                class="rounded-lg border border-border/60 bg-muted/50 p-4"
              >
                <div class="flex items-start justify-between gap-3">
                  <div class="min-w-0">
                  ...
                  <span class="inline-flex shrink-0 rounded-md bg-card px-2 py-1 font-mono text-xs font-medium text-muted-foreground shadow-sm">
          ...
          <div v-else class="grid grid-cols-1 gap-5 lg:grid-cols-2">
            ...
            <div class="rounded-lg border border-border/60 p-4 text-sm text-muted-foreground lg:col-span-2">
              <div class="flex flex-wrap items-center gap-3">
                <Icon name="database" size="md" class="text-muted-foreground/80" />
      ...
      >
        <div v-if="inputDetailRow" class="space-y-5">
          <div class="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-4">
            <div class="rounded-lg border border-border/60 bg-muted/50 p-4">
          ...
          <div class="rounded-xl border border-border/60 bg-card p-4 shadow-sm">
            ...
            <pre class="mt-4 max-h-[420px] overflow-auto whitespace-pre-wrap break-words rounded-lg bg-muted p-4 text-sm leading-6 text-foreground shadow-inner">{{ inputDetailText }}</pre>
          </div>
        </div>
...
const keywordNoticeTones = {
  info: {
    ...
    toneClass: 'border-brand/20 bg-brand/10',
    iconClass: 'mt-0.5 flex-shrink-0 text-brand',
...
function resultBadgeClass(row: ContentModerationLog): string {
  if (row.action === 'block' || row.action === 'keyword_block' || row.action === 'cyber_policy') return 'bg-red-50 text-red-700 dark:bg-red-900/25 dark:text-red-300'
  if (row.action === 'error' || row.error) return 'bg-amber-50 text-amber-700 dark:bg-amber-900/25 dark:text-amber-300'
  if (row.flagged) return 'bg-pink-50 text-pink-700 dark:bg-pink-900/25 dark:text-pink-300'
...
function workerSlotClass(state: WorkerSlotState): string {
  ...
  return 'border-border/60 bg-card text-muted-foreground/80'
}
