    ...
    <div class="space-y-6">
      ...
      <div class="card p-6">
        <div class="mb-4 flex flex-wrap items-center justify-between gap-3">
          <div>
            ...
            <p class="mt-1 text-sm text-muted-foreground">
              ...
              <button type="button" class="text-brand underline transition-opacity hover:opacity-80" @click="showR2Guide = true">Cloudflare R2</button>
        ...
        <div class="grid grid-cols-1 gap-3 md:grid-cols-2">
          <div>
            <label class="mb-1 block text-xs font-medium text-muted-foreground">{{ t('admin.backup.s3.endpoint') }}</label>
            <input v-model="s3Form.endpoint" class="input w-full" placeholder="https://<account_id>.r2.cloudflarestorage.com" />
          </div>
          <div>
            <label class="mb-1 block text-xs font-medium text-muted-foreground">{{ t('admin.backup.s3.region') }}</label>
            <input v-model="s3Form.region" class="input w-full" placeholder="auto" />
          </div>
          <div>
            <label class="mb-1 block text-xs font-medium text-muted-foreground">{{ t('admin.backup.s3.bucket') }}</label>
            <input v-model="s3Form.bucket" class="input w-full" />
          </div>
          <div>
            <label class="mb-1 block text-xs font-medium text-muted-foreground">{{ t('admin.backup.s3.prefix') }}</label>
            <input v-model="s3Form.prefix" class="input w-full" placeholder="backups/" />
          </div>
          <div>
            <label class="mb-1 block text-xs font-medium text-muted-foreground">{{ t('admin.backup.s3.accessKeyId') }}</label>
            <input v-model="s3Form.access_key_id" class="input w-full" />
          </div>
          <div>
            <label class="mb-1 block text-xs font-medium text-muted-foreground">{{ t('admin.backup.s3.secretAccessKey') }}</label>
      ...
      <div class="card p-6">
        ...
        <div class="mt-3 grid grid-cols-1 gap-3 md:grid-cols-2">
          <div>
            <label class="mb-1 block text-xs font-medium text-muted-foreground">{{ t('admin.backup.imageStorage.bucket') }}</label>
          ...
          <div>
            <label class="mb-1 block text-xs font-medium text-muted-foreground">{{ t('admin.backup.imageStorage.prefix') }}</label>
          ...
          <div>
            <label class="mb-1 block text-xs font-medium text-muted-foreground">{{ t('admin.backup.imageStorage.publicBaseUrl') }}</label>
          ...
          <div>
            <label class="mb-1 block text-xs font-medium text-muted-foreground">{{ t('admin.backup.imageStorage.presignExpiryHours') }}</label>
      ...
      <div class="card p-6">
        ...
        <div class="grid grid-cols-1 gap-3 md:grid-cols-2">
          ...
          <div>
            <label class="mb-1 block text-xs font-medium text-muted-foreground">{{ t('admin.backup.schedule.cronExpr') }}</label>
          ...
          <div>
            <label class="mb-1 block text-xs font-medium text-muted-foreground">{{ t('admin.backup.schedule.retainDays') }}</label>
          ...
          <div>
            <label class="mb-1 block text-xs font-medium text-muted-foreground">{{ t('admin.backup.schedule.retainCount') }}</label>
      ...
      <div class="card p-6">
        <div class="mb-4 flex flex-wrap items-center justify-between gap-3">
          ...
          <div class="flex flex-wrap items-center gap-2">
            <div class="flex items-center gap-1">
              <label class="text-xs text-muted-foreground">{{ t('admin.backup.operations.expireDays') }}</label>
        ...
        <div class="overflow-x-auto">
          <table class="w-full min-w-[800px] text-sm">
            ...
            <tbody>
              <tr v-for="record in backups" :key="record.id" class="border-b border-border/60 align-top">
    ...
    <teleport to="body">
      <transition name="modal">
        <div v-if="showR2Guide" class="fixed inset-0 z-50 flex items-center justify-center p-4" @mousedown.self="showR2Guide = false">
          ...
          <div class="relative max-h-[85vh] w-full max-w-2xl overflow-y-auto rounded-xl bg-card p-6 shadow-2xl">
            <button type="button" class="absolute right-4 top-4 text-muted-foreground transition-colors hover:text-foreground" @click="showR2Guide = false">
            ...
            <div class="mb-5">
              <h3 class="mb-2 flex items-center gap-2 text-sm font-semibold text-foreground">
                <span class="flex h-6 w-6 items-center justify-center rounded-full bg-brand/10 text-xs font-bold text-brand">1</span>
                {{ t('admin.backup.r2Guide.step1.title') }}
              </h3>
              <ol class="ml-8 list-decimal space-y-1 text-sm text-foreground">
            ...
            <div class="mb-5">
              <h3 class="mb-2 flex items-center gap-2 text-sm font-semibold text-foreground">
                <span class="flex h-6 w-6 items-center justify-center rounded-full bg-brand/10 text-xs font-bold text-brand">2</span>
            ...
            <div class="mb-5">
              <h3 class="mb-2 flex items-center gap-2 text-sm font-semibold text-foreground">
                <span class="flex h-6 w-6 items-center justify-center rounded-full bg-brand/10 text-xs font-bold text-brand">3</span>
                {{ t('admin.backup.r2Guide.step3.title') }}
              </h3>
              <p class="ml-8 text-sm text-foreground">{{ t('admin.backup.r2Guide.step3.desc') }}</p>
            ...
            <div class="mb-5">
              <h3 class="mb-2 flex items-center gap-2 text-sm font-semibold text-foreground">
                <span class="flex h-6 w-6 items-center justify-center rounded-full bg-brand/10 text-xs font-bold text-brand">4</span>
                {{ t('admin.backup.r2Guide.step4.title') }}
              </h3>
              <div class="ml-8 overflow-hidden rounded-lg border border-border">
                <table class="w-full text-sm">
                  <tbody>
                    <tr v-for="(row, i) in r2ConfigRows" :key="i" class="border-b border-border/60 last:border-0">
                      <td class="whitespace-nowrap bg-muted/50 px-3 py-2 font-medium text-foreground">{{ row.field }}</td>
            ...
            <!-- Free tier note -->
            <div class="rounded-lg bg-emerald-50 p-3 text-xs text-emerald-700 dark:bg-emerald-900/20 dark:text-emerald-300">
    ...
    <teleport to="body">
      <transition name="modal">
        ...
        >
          ...
          <div class="relative max-h-[85vh] w-full max-w-lg overflow-y-auto rounded-xl bg-card p-6 shadow-2xl">
            <button
              type="button"
            ...
            <p class="mb-4 text-sm text-muted-foreground">{{ t('admin.backup.actions.downloadPartsHint') }}</p>
            <div class="space-y-2">
              <div
              ...
              >
                <span class="text-sm text-foreground">
                  ...
                  <span class="ml-2 text-xs text-muted-foreground">{{ formatSize(part.size_bytes) }}</span>
                </span>
