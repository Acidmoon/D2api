<template>
  <AppLayout>
    <TablePageLayout>
      <!-- Page header + toolbar: Search, Filters, and Actions -->
      <template #filters>
        <div>
          ...
          <div class="flex flex-wrap items-center justify-end gap-2">
            ...
            <div class="flex items-center gap-2 md:contents">
              ...
              <button
                ...
                :class="isUsageSortActive('usage', 'last7d') ? 'text-brand' : ''"
                ...
                >
                  ...
                  >
                    ...
                    <Icon
                      ...
                      name="check"
                      size="sm"
                      :class="isForcedVisibleColumn(col.key) ? 'text-muted-foreground opacity-60' : 'text-brand'"
                      :stroke-width="2"
                    />
                  </button>
                </div>
        ...
        >
          <template #cell-email="{ value }">
            <div class="flex items-center gap-2">
              <div
                class="flex h-8 w-8 items-center justify-center rounded-full bg-brand/10"
              >
          ...
          <template #cell-groups="{ row }">
            <div v-if="allGroups.length > 0" class="flex flex-col gap-1">
              ...
              >
                <Icon name="shield" size="xs" class="h-3.5 w-3.5 text-brand" />
                <span class="font-medium text-brand">{{ getUserGroups(row).exclusive.length }}</span>
                ...
                <div
                  ...
                  class="pointer-events-none absolute left-0 top-full z-50 mt-1.5 rounded bg-foreground px-2.5 py-1.5 text-xs text-background opacity-0 shadow-lg transition-opacity duration-75 group-hover/ex:opacity-100"
                >
                  <div class="absolute bottom-full left-4 border-4 border-transparent border-b-foreground"></div>
                ...
                <div
                  v-if="expandedGroupUserId === row.id"
                  class="absolute left-0 top-full z-50 mt-1.5 min-w-[160px] overflow-hidden rounded-lg border border-border bg-card py-1 text-xs shadow-lg"
                >
                  ...
                  <div
                    ...
                    :key="g.id"
                    class="flex cursor-pointer items-center gap-2 px-3 py-2 text-foreground transition-colors hover:bg-secondary hover:text-brand"
              ...
              >
                ...
                <span class="font-medium text-foreground">{{ getUserGroups(row).publicGroups.length }}</span>
                <span class="text-muted-foreground">{{ t('admin.users.publicLabel') }}</span>
                <!-- Tooltip: 向下弹出 -->
                <div class="pointer-events-none absolute left-0 top-full z-50 mt-1.5 rounded bg-foreground px-2.5 py-1.5 text-xs text-background opacity-0 shadow-lg transition-opacity duration-75 group-hover/pub:opacity-100">
                  <div class="absolute bottom-full left-4 border-4 border-transparent border-b-foreground"></div>
                  <div class="flex flex-col gap-0.5 whitespace-nowrap">
          ...
          <template #cell-subscriptions="{ row }">
            ...
            <span
              v-else
              class="inline-flex items-center gap-1.5 rounded-md bg-secondary px-2 py-1 text-xs text-muted-foreground"
            >
          ...
          <template #cell-balance="{ value, row }">
            <div class="flex items-center gap-2">
              <div class="group relative">
                <button
                  class="font-medium text-foreground underline decoration-dashed decoration-input underline-offset-4 transition-colors hover:text-brand"
                  @click="handleBalanceHistory(row)"
                >
                  ${{ value.toFixed(2) }}
                </button>
                <!-- Instant tooltip -->
                <div class="pointer-events-none absolute bottom-full left-1/2 z-50 mb-1.5 -translate-x-1/2 whitespace-nowrap rounded bg-foreground px-2 py-1 text-xs text-background opacity-0 shadow-lg transition-opacity duration-75 group-hover:opacity-100">
              ...
              <button
                ...
                class="rounded-full px-2 py-0.5 text-xs font-medium text-semantic-success transition-colors hover:bg-secondary"
          ...
          <template #cell-balance_platform_quota="{ row }">
            <button
              type="button"
              class="block text-left underline decoration-dashed decoration-input underline-offset-4 transition-colors hover:decoration-brand"
          ...
          >
            <div class="flex items-center gap-1.5">
              ...
              <div class="usage-sort-trigger relative">
                <button
                  type="button"
                  class="flex items-center gap-1 rounded px-1 py-0.5 transition-colors hover:bg-secondary"
                ...
                >
                  <button
                    ...
                    :key="metric"
                    type="button"
                    class="flex w-full items-center justify-between gap-3 rounded-lg px-3 py-1.5 text-left text-xs normal-case tracking-normal hover:bg-secondary"
            ...
            <div class="flex flex-wrap items-center gap-1.5">
              <span
                :class="[
                  ...
                  value === 'active' ? 'bg-[var(--nm-success)]' : 'bg-[var(--nm-danger)]'
                ]"
              ></span>
          ...
          <template #cell-actions="{ row }">
            <div class="flex items-center gap-3 whitespace-nowrap text-sm">
              ...
              <button
                ...
                class="font-medium text-brand transition-opacity hover:opacity-80 hover:underline"
              >
                {{ t('common.edit') }}
              </button>
              ...
              <button
                ...
                class="action-menu-trigger font-medium transition-opacity hover:opacity-80 hover:underline"
                :class="activeMenuId === row.id ? 'text-foreground' : 'text-muted-foreground'"
              >
                {{ t('common.more') }}
              </button>
            </div>
    ...
    <Teleport to="body">
      <div
        ...
        class="action-menu-content dropdown fixed z-[9999] w-48 overflow-hidden"
          ...
          <template v-for="user in users" :key="user.id">
            <template v-if="user.id === activeMenuId">
              ...
              <button
                ...
                class="flex w-full items-center gap-2 rounded-lg px-4 py-2 text-sm text-foreground hover:bg-secondary"
              >
              ...
              <button
                ...
                class="flex w-full items-center gap-2 rounded-lg px-4 py-2 text-sm text-foreground hover:bg-secondary"
              >
                <Icon name="plus" size="sm" class="text-semantic-success" :stroke-width="2" />
                {{ t('admin.users.deposit') }}
              </button>
              ...
              <button
                ...
                class="flex w-full items-center gap-2 rounded-lg px-4 py-2 text-sm text-foreground hover:bg-secondary"
              >
                <svg class="h-4 w-4 text-semantic-warning" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              ...
              <button
                ...
                class="flex w-full items-center gap-2 rounded-lg px-4 py-2 text-sm text-semantic-danger hover:bg-secondary"
              >
