      ...
      <template #filters>
        <div>
          ...
          <div class="page-header">
            <h1 class="page-title">{{ t('admin.channels.title') }}</h1>
            <p class="page-description">{{ t('admin.channels.description') }}</p>
          </div>
          ...
          <template #cell-actions="{ row }">
            <div class="flex items-center gap-3 whitespace-nowrap text-sm">
              ...
              <button
                ...
                class="font-medium text-destructive transition-opacity hover:opacity-80 hover:underline"
              >
      ...
      <div class="channel-dialog-body">
        ...
        <form id="channel-form" @submit.prevent="handleSubmit" class="flex-1 overflow-y-auto pt-4">
          ...
          <div v-show="activeTab === 'basic'" class="space-y-5">
            <!-- Name -->
            <div>
              <label class="input-label">{{ t('admin.channels.form.name', 'Name') }} <span class="text-destructive">*</span></label>
              <input
                v-model="form.name"
                type="text"
                required
            ...
            <div class="space-y-3">
              ...
              <div class="flex flex-wrap gap-2">
                <label
                  ...
                  :key="p"
                  class="inline-flex cursor-pointer items-center gap-1.5 rounded-full border px-3 py-1.5 text-sm transition-colors"
                ...
                >
                  <input
                    ...
                    class="h-3.5 w-3.5 rounded border-input text-brand focus:ring-ring"
          ...
          >
            ...
            <div>
              ...
              <div class="max-h-40 overflow-auto rounded-lg border border-border bg-secondary p-2">
                ...
                <div v-else class="flex flex-wrap gap-1">
                  <label
                    ...
                    :key="group.id"
                    class="inline-flex cursor-pointer items-center gap-1.5 rounded-full border border-border px-2 py-1 text-xs transition-colors hover:bg-secondary"
                    :class="[
                      section.group_ids.includes(group.id) ? 'border-brand/30 bg-brand/10' : '',
                  ...
                  >
                    <input
                      ...
                      class="h-3 w-3 rounded border-input text-brand focus:ring-ring"
            ...
            <div v-if="section.platform === 'openai'" class="border-t border-border pt-3">
              <div class="flex items-center justify-between gap-4">
                <div>
                  ...
                  <p class="mt-0.5 text-[11px] text-semantic-warning">
            ...
            <div>
              <div class="mb-1 flex items-center justify-between">
                ...
                <button type="button" @click="addMappingEntry(sIdx)" class="text-xs font-medium text-brand transition-opacity hover:opacity-80">
                  + {{ t('common.add', 'Add') }}
                </button>
              </div>
              <div
              ...
              <div class="mb-1 flex items-center justify-between">
                ...
                <div class="flex items-center gap-2">
                  <button
                    ...
                    class="text-xs text-muted-foreground transition-opacity hover:opacity-80 disabled:opacity-50"
                  >
                    {{ syncingPlatform === section.platform ? t('admin.channels.form.syncingModels') : t('admin.channels.form.syncLatestModels') }}
                  </button>
                  <button type="button" @click="addPricingEntry(sIdx)" class="text-xs font-medium text-brand transition-opacity hover:opacity-80">
                    + {{ t('common.add', 'Add') }}
                  </button>
                </div>
              </div>
            ...
            <div class="mt-4 border-t border-border pt-4 space-y-3">
              <div class="flex items-center justify-between">
                ...
                <button
                  ...
                  class="rounded-full border border-brand/30 px-3 py-1 text-xs font-medium text-brand transition-colors hover:bg-brand/10"
                >
              ...
              >
                <div class="flex items-center justify-between">
                  <input
                    ...
                    class="bg-transparent text-sm font-medium text-foreground placeholder:text-muted-foreground outline-none"
                  />
                  <button type="button" @click="removeAccountStatsRule(sIdx, ruleIndex)" class="text-xs font-medium text-destructive transition-opacity hover:opacity-80">
                    {{ t('common.delete') }}
                  </button>
                </div>
                ...
                <div>
                  ...
                  <div class="mt-1 flex flex-wrap gap-1">
                    <label
                      ...
                      :key="gid"
                      class="inline-flex cursor-pointer items-center gap-1 rounded-full border px-2 py-1 text-xs transition-colors"
                      :class="rule.group_ids.includes(gid)
                        ? 'border-brand/30 bg-brand/10'
                        : 'border-border hover:bg-secondary'"
                    >
                      <input type="checkbox" :checked="rule.group_ids.includes(gid)" class="h-3 w-3 rounded border-input text-brand focus:ring-ring" @change="rule.group_ids.includes(gid) ? rule.group_ids.splice(rule.group_ids.indexOf(gid), 1) : rule.group_ids.push(gid)" />
                ...
                <div>
                  ...
                  <div class="mt-1 flex flex-wrap gap-1">
                    ...
                    >
                      ...
                      <button type="button" @click="removeRuleAccount(rule, accountId)" class="transition-colors hover:text-destructive">
                        <Icon name="x" size="xs" />
                      </button>
                    </span>
                  </div>
                  ...
                  <div class="relative mt-1 rule-account-search-container">
                    ...
                    <div
                      ...
                      class="absolute z-50 mt-1 max-h-48 w-full overflow-auto rounded-lg border border-border bg-popover shadow-lg"
                    >
                      <button
                        ...
                        class="w-full px-3 py-2 text-left text-sm transition-colors hover:bg-secondary"
                ...
                <div>
                  <div class="mb-1 flex items-center justify-between">
                    ...
                    <button type="button" @click="addRulePricingEntry(sIdx, ruleIndex)" class="text-xs font-medium text-brand transition-opacity hover:opacity-80">
                      + {{ t('common.add') }}
                    </button>
                  </div>
