  ...
  <AppLayout>
    <TablePageLayout>
      <template #filters>
        ...
        <div class="page-header">
          <h1 class="page-title">{{ t('admin.accounts.title') }}</h1>
          <p class="page-description">{{ t('admin.accounts.description') }}</p>
        </div>
            ...
            <template #after>
              ...
              <div class="relative" ref="autoRefreshDropdownRef">
                ...
                <div
                  ...
                  class="dropdown right-0 top-full mt-1.5 w-56"
                >
                  <div class="p-1">
                    <button
              ...
              <div class="relative" ref="accountToolsDropdownRef">
                ...
                <Teleport to="body">
                  <div
                    ...
                    class="fixed z-[9999] origin-top-right overflow-hidden rounded-2xl border border-border bg-popover shadow-lg"
                    :style="accountToolsDropdownStyle"
                    @click.stop
                  >
                    <div class="overflow-y-auto p-1.5" :style="{ maxHeight: `${accountToolsDropdownPosition.maxHeight}px` }">
                      <div class="px-2 py-2">
                      ...
                      <div class="grid grid-cols-1 gap-1">
                        <button
                          ...
                          class="flex w-full items-center justify-between rounded-lg px-2.5 py-1.5 text-sm text-foreground transition-colors hover:bg-secondary"
                        >
        ...
        <div
          ...
          class="mt-2 flex items-center justify-between rounded-xl border border-border bg-secondary px-3 py-2 text-sm text-foreground"
        >
          <span class="inline-flex items-center gap-2 text-semantic-warning">
      ...
      <template #table>
        ...
        >
          <template #header-select>
            <input
              type="checkbox"
              class="h-4 w-4 cursor-pointer rounded border-input text-brand focus:ring-brand"
          ...
          <template #cell-select="{ row }">
            <input type="checkbox" :checked="isSelected(row.id)" @change="toggleSel(row.id)" class="rounded border-input text-brand focus:ring-brand" />
          </template>
          ...
          <template #cell-notes="{ value }">
            <span v-if="value" :title="value" class="block max-w-xs truncate text-sm text-muted-foreground">{{ value }}</span>
            <span v-else class="text-sm text-muted-foreground/70">-</span>
          </template>
          ...
          <template #cell-schedulable="{ row }">
            <button @click="handleToggleSchedulable(row)" :disabled="togglingSchedulable === row.id" class="relative inline-flex h-5 w-9 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none focus:ring-2 focus:ring-brand focus:ring-offset-2 focus:ring-offset-background disabled:cursor-not-allowed disabled:opacity-50" :class="[row.schedulable ? 'bg-brand hover:bg-brand/90' : 'bg-muted hover:bg-secondary']" :title="row.schedulable ? t('admin.accounts.schedulableEnabled') : t('admin.accounts.schedulableDisabled')">
              <span class="pointer-events-none inline-block h-4 w-4 transform rounded-full bg-white shadow ring-0 dark:bg-background transition duration-200 ease-in-out" :class="[row.schedulable ? 'translate-x-4' : 'translate-x-0']" />
            </button>
          </template>
          ...
          <template #cell-proxy="{ row }">
            <div class="flex flex-col gap-1">
              ...
              <div v-if="row.proxy_fallback_origin_id" class="flex items-center gap-1">
                ...
                <button class="rounded-full border border-border px-2 py-0.5 text-xs text-muted-foreground transition-colors hover:bg-secondary hover:text-foreground" @click="onRevertFallback(row)">{{ t('admin.accounts.revertProxy') }}</button>
              </div>
            </div>
          </template>
          ...
          <template #cell-scheduler_score="{ row }">
            <div v-if="getSchedulerScoreRows(row).length" class="flex min-w-[7rem] flex-col gap-0.5 font-mono text-[11px] leading-4">
              ...
              >
                <span class="max-w-[4.75rem] truncate text-muted-foreground">{{ formatSchedulerScoreGroup(score) }}</span>
          ...
          <template #cell-actions="{ row }">
            <div class="flex items-center gap-1">
              <button @click="handleEdit(row)" class="flex flex-col items-center gap-0.5 rounded-lg p-1.5 text-muted-foreground transition-colors hover:bg-secondary hover:text-brand">
                <svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="1.5"><path stroke-linecap="round" stroke-linejoin="round" d="M16.862 4.487l1.687-1.688a1.875 1.875 0 112.652 2.652L10.582 16.07a4.5 4.5 0 01-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 011.13-1.897l8.932-8.931zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0115.75 21H5.25A2.25 2.25 0 013 18.75V8.25A2.25 2.25 0 015.25 6H10" /></svg>
                <span class="text-xs">{{ t('common.edit') }}</span>
              </button>
              <button @click="handleDelete(row)" class="flex flex-col items-center gap-0.5 rounded-lg p-1.5 text-muted-foreground transition-colors hover:bg-destructive/10 hover:text-destructive">
                <svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="1.5"><path stroke-linecap="round" stroke-linejoin="round" d="M14.74 9l-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 01-2.244 2.077H8.084a2.25 2.25 0 01-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 00-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 013.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 00-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 00-7.5 0" /></svg>
                <span class="text-xs">{{ t('common.delete') }}</span>
              </button>
              <button @click="openMenu(row, $event)" class="flex flex-col items-center gap-0.5 rounded-lg p-1.5 text-muted-foreground transition-colors hover:bg-secondary hover:text-foreground">
    ...
    <ConfirmDialog :show="showExportDataDialog" :title="t('admin.accounts.dataExport')" :message="t('admin.accounts.dataExportConfirmMessage')" :confirm-text="t('admin.accounts.dataExportConfirm')" :cancel-text="t('common.cancel')" @confirm="handleExportData" @cancel="showExportDataDialog = false">
      <label class="flex items-center gap-2 text-sm text-foreground">
        <input type="checkbox" class="h-4 w-4 rounded border-input text-brand focus:ring-brand" v-model="includeProxyOnExport" />
...
// 全局探测开关由系统设置维护(见 SettingsView),此处只读 enabled 供单元格显示;
// undefined 表示尚未加载,单元格按默认开启渲染。
const upstreamBillingProbeGloballyEnabled = ref<boolean | undefined>(undefined)
const upstreamBillingNow = ref(Date.now())
// 按 upstream_billing_rate 排序时,每分钟至多自动刷新一次,该变量记录上次刷新的分钟戳。
...
// 探测快照按服务端时间排序,本地打补丁后排序会失效;这里在常规加载路径上记录
// "本分钟已刷新",避免随后的 watch(upstreamBillingNow) 在同一分钟内重复拉取。
...
// 按 upstream_billing_rate 排序时,探测结果随时在后台更新,需定期重拉以维持排序正确;
// force 用于单个/批量探测成功后立刻刷新,绕过每分钟一次的节流。
...
// API Key 账号名称可点击跳转上游站点主页(取 base_url 的 origin);sanitizeUrl 拦截 javascript: 等危险协议。
...
// 批量探测上限 20 个(与后端一致),结果直接打补丁进列表;排序中则强制刷新一次。
...
// 探测快照存放在 extra.upstream_billing_probe;补丁后若正在按该列排序,标记本分钟已刷新,
// 防止 watch(upstreamBillingNow) 立刻又触发一次全量 reload(由调用方决定是否 force 刷新)。
...
const handleExportData = async () => {
  ...
  try {
    // 后端对导出强制 step-up 2FA:经 useStepUp 包装,STEP_UP_REQUIRED 时弹 TOTP 窗后自动重试一次。
...
// 服务端复制(凭据不回传浏览器);副本默认暂停调度。同一账号并发点击只发一次请求。
...
.account-tools-menu-item {
  @apply flex w-full items-center gap-2.5 rounded-lg px-2.5 py-2 text-sm text-foreground transition-colors hover:bg-secondary hover:text-accent-foreground;
}

.account-tools-menu-icon {
  @apply inline-flex h-7 w-7 flex-shrink-0 items-center justify-center rounded-full;
}
</style>
