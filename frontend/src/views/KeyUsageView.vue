<template>
  <div class="relative flex min-h-screen flex-col" style="background-color: var(--nm-bg)">
        ...
        <div class="flex gap-3">
          <div class="flex-1 relative">
            ...
            <button
              ...
              class="absolute right-2 top-1/2 flex h-9 w-9 -translate-y-1/2 items-center justify-center text-muted-foreground transition-colors hover:text-foreground"
            >
          ...
          <div class="flex flex-wrap items-center gap-2 justify-center">
            ...
            <div v-if="currentRange === 'custom'" class="flex items-center gap-2 ml-1">
              <input
                ...
                type="date"
                class="input min-h-0 px-2 py-1.5 text-xs"
              />
      ...
      <div v-if="showResults">
        ...
        <div v-if="showLoading" class="space-y-6">
          <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div class="swiss-panel p-8">
              ...
              <div class="flex justify-center"><div class="skeleton h-44 w-44"></div></div>
            </div>
            <div class="swiss-panel p-8">
              <div class="skeleton h-5 w-24 mb-6"></div>
              <div class="flex justify-center"><div class="skeleton h-44 w-44"></div></div>
        ...
        <div v-else-if="resultData" class="space-y-6">
          ...
          <div v-if="statusInfo" class="fade-up flex items-center justify-center mb-2">
            <div class="badge inline-flex items-center gap-2 px-4 py-2">
              <span
                class="h-2.5 w-2.5 pulse-dot"
                style="border-radius: var(--nm-radius-sm)"
                :style="{ backgroundColor: statusInfo.isActive ? 'var(--nm-success)' : 'var(--nm-danger)' }"
              ></span>
          ...
          <div v-if="ringItems.length > 0" :class="ringGridClass">
            <div
              ...
              :key="i"
              class="fade-up card p-8 transition-all duration-300"
                ...
                <div class="relative">
                  <svg class="w-44 h-44" viewBox="0 0 160 160">
                    ...
                    <circle
                      ...
                      :stroke="ringStrokeColor(ring.pct)"
                  ...
                  <div class="absolute inset-0 flex flex-col items-center justify-center">
                    <template v-if="ring.isBalance">
                      <span class="text-2xl font-bold tabular-nums" style="color: var(--nm-accent-text)">
                        {{ ring.amount }}
                      </span>
                    </template>
                    <template v-else>
          ...
          >
            ...
            <div class="divide-y divide-border">
              ...
              >
                <div class="flex items-center gap-3">
                  <div class="swiss-stat-icon" :class="row.iconBg">
                    <svg
                      class="w-4 h-4"
                      :class="row.iconColor"
          ...
          >
            <div class="flex flex-col gap-3 px-8 py-5 border-b border-border sm:flex-row sm:items-center sm:justify-between">
              ...
              <div class="tabs">
                <button
                  ...
                  class="tab min-w-12 px-3 py-1.5 text-xs"
            ...
            <div v-if="dailyUsageRows.length > 0" class="overflow-x-auto">
              <table class="table">
                <thead>
                  <tr>
                    <th class="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider">{{ t('keyUsage.date') }}</th>
                    <th class="px-4 py-3 text-right text-xs font-semibold uppercase tracking-wider">{{ t('keyUsage.requests') }}</th>
                    <th class="px-4 py-3 text-right text-xs font-semibold uppercase tracking-wider">{{ t('keyUsage.inputTokens') }}</th>
                    <th class="px-4 py-3 text-right text-xs font-semibold uppercase tracking-wider">{{ t('keyUsage.outputTokens') }}</th>
                    <th class="px-4 py-3 text-right text-xs font-semibold uppercase tracking-wider">{{ t('keyUsage.cacheReadTokens') }}</th>
                    <th class="px-4 py-3 text-right text-xs font-semibold uppercase tracking-wider">{{ t('keyUsage.cacheWriteTokens') }}</th>
                    <th class="px-4 py-3 text-right text-xs font-semibold uppercase tracking-wider">{{ t('keyUsage.cost') }}</th>
                  </tr>
                </thead>
                <tbody>
                  <tr
                    ...
                    class=""
                  >
                    <td class="px-4 py-3 text-sm font-medium whitespace-nowrap">{{ row.date }}</td>
                    <td class="px-4 py-3 text-sm tabular-nums text-right">{{ fmtNum(row.requests) }}</td>
                    <td class="px-4 py-3 text-sm tabular-nums text-right">{{ fmtNum(row.input_tokens) }}</td>
                    <td class="px-4 py-3 text-sm tabular-nums text-right">{{ fmtNum(row.output_tokens) }}</td>
                    <td class="px-4 py-3 text-sm tabular-nums text-right">{{ fmtNum(row.cache_read_tokens) }}</td>
                    <td class="px-4 py-3 text-sm tabular-nums text-right">{{ fmtNum(row.cache_write_tokens) }}</td>
                    <td class="px-4 py-3 text-sm tabular-nums text-right font-medium">{{ usd(row.actual_cost != null ? row.actual_cost : row.cost) }}</td>
                  </tr>
                </tbody>
              </table>
            </div>
          ...
          >
            ...
            <div class="overflow-x-auto">
              <table class="table">
                <thead>
                  <tr>
                    <th class="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider">{{ t('keyUsage.model') }}</th>
                    <th class="px-4 py-3 text-right text-xs font-semibold uppercase tracking-wider">{{ t('keyUsage.requests') }}</th>
                    <th class="px-4 py-3 text-right text-xs font-semibold uppercase tracking-wider">{{ t('keyUsage.inputTokens') }}</th>
                    <th class="px-4 py-3 text-right text-xs font-semibold uppercase tracking-wider">{{ t('keyUsage.outputTokens') }}</th>
                    <th class="px-4 py-3 text-right text-xs font-semibold uppercase tracking-wider">{{ t('keyUsage.cacheCreationTokens') }}</th>
                    <th class="px-4 py-3 text-right text-xs font-semibold uppercase tracking-wider">{{ t('keyUsage.cacheReadTokens') }}</th>
                    <th class="px-4 py-3 text-right text-xs font-semibold uppercase tracking-wider">{{ t('keyUsage.totalTokens') }}</th>
                    <th class="px-4 py-3 text-right text-xs font-semibold uppercase tracking-wider">{{ t('keyUsage.cost') }}</th>
                  </tr>
                </thead>
                <tbody>
                  <tr
                    ...
                    class=""
                  >
                    <td class="px-4 py-3 text-sm font-medium whitespace-nowrap">{{ m.model || '-' }}</td>
                    <td class="px-4 py-3 text-sm tabular-nums text-right">{{ fmtNum(m.requests) }}</td>
                    <td class="px-4 py-3 text-sm tabular-nums text-right">{{ fmtNum(m.input_tokens) }}</td>
                    <td class="px-4 py-3 text-sm tabular-nums text-right">{{ fmtNum(m.output_tokens) }}</td>
                    <td class="px-4 py-3 text-sm tabular-nums text-right">{{ fmtNum(m.cache_creation_tokens) }}</td>
                    <td class="px-4 py-3 text-sm tabular-nums text-right">{{ fmtNum(m.cache_read_tokens) }}</td>
                    <td class="px-4 py-3 text-sm tabular-nums text-right">{{ fmtNum(m.total_tokens) }}</td>
                    <td class="px-4 py-3 text-sm tabular-nums text-right font-medium">{{ usd(m.actual_cost != null ? m.actual_cost : m.cost) }}</td>
                  </tr>
                </tbody>
              </table>
            </div>
...
const ringTrackColor = computed(() => isDark.value ? '#34342f' : '#ddddd6')
...
function ringStrokeColor(pct: number): string {
  if (pct > 90) return isDark.value ? '#ff8f90' : '#b4232a'
  if (pct > 70) return isDark.value ? '#f0be57' : '#9a6700'
  return isDark.value ? '#45c3b5' : '#0f766e'
}
...
function getUsageColor(pct: number): string {
  if (pct > 90) return 'text-semantic-danger'
  if (pct > 70) return 'text-semantic-warning'
...
const detailRows = computed<DetailRow[]>(() => {
  ...
  if (data.mode === 'quota_limited') {
    if (data.quota) {
      const remainColor = data.quota.remaining <= 0 ? 'text-semantic-danger'
        : data.quota.remaining < data.quota.limit * 0.1 ? 'text-semantic-warning'
        : 'text-semantic-success'
      rows.push({
        iconBg: '', iconColor: 'text-semantic-success', iconSvg: ICON_SHIELD,
    ...
    if (data.expires_at) {
      ...
      rows.push({
        iconBg: '', iconColor: 'text-semantic-warning', iconSvg: ICON_CALENDAR,
    ...
    if (data.rate_limits) {
      ...
      for (const rl of data.rate_limits) {
        ...
        rows.push({
          iconBg: '', iconColor: 'text-semantic-info', iconSvg: ICON_DOLLAR,
    ...
    rows.push({
      iconBg: '', iconColor: 'text-semantic-success', iconSvg: ICON_CHECK,
    ...
    if (data.subscription) {
      ...
      if (sub.monthly_limit_usd > 0) {
        ...
        rows.push({
          iconBg: '', iconColor: 'text-semantic-success', iconSvg: ICON_DOLLAR,
    ...
    const remainColor = data.remaining != null
      ? (data.remaining <= 0 ? 'text-semantic-danger' : data.remaining < 10 ? 'text-semantic-warning' : 'text-semantic-success')
      : ''
    rows.push({
      iconBg: '', iconColor: 'text-semantic-success', iconSvg: ICON_SHIELD,
