-- Group per-model pricing and the long-context switch live on the API-key
-- auth snapshot. Extend the durable invalidation trigger so out-of-band
-- edits (direct SQL, crash between update and app-level invalidation) cannot
-- leave cached snapshots using stale pricing. Normal admin saves already
-- invalidate via InvalidateAuthCacheByGroupID; this trigger is the durable
-- backstop. Based on 193_group_profit_control_auth_cache_invalidation.sql.
