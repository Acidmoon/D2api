...
func createGroupRecord(ctx context.Context, client *dbent.Client, groupIn *service.Group) error {
	...
	builder := client.Group.Create().
		...
		SetNillableRechargeMultiplier(groupIn.RechargeMultiplier).
		...
		SetUnavailableAlertEnabled(groupIn.UnavailableAlertEnabled).
...
func (r *groupRepository) Update(ctx context.Context, groupIn *service.Group) error {
	...
	if groupIn.RechargeMultiplier != nil {
		builder = builder.SetRechargeMultiplier(*groupIn.RechargeMultiplier)
	} else {
		builder = builder.ClearRechargeMultiplier()
	}
...
func (r *groupRepository) DeleteCascade(ctx context.Context, id int64) ([]int64, error) {
	...
	// A subscription wallet is independent of its optional source group. Deleting
	// a route group must never revoke users' active subscription wallets.
