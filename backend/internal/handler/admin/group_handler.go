...
func sanitizeCreateGroupRequestForSimpleMode(req *CreateGroupRequest) {
	...
	// 简单模式不开放长上下文阶梯计价：显式传 false，避免 Create 路径的缺省 true 回退。
	disabledLongContextPricing := false
	allowed.LongContextPricingEnabled = &disabledLongContextPricing
	*req = allowed
}
...
func (h *GroupHandler) Create(c *gin.Context) {
	...
	group, err := h.adminService.CreateGroup(c.Request.Context(), &service.CreateGroupInput{
		...
		UnavailableAlertEnabled:         req.UnavailableAlertEnabled,
