...
func TestSettingHandler_GetSettings_InjectsAuthSourceDefaults(t *testing.T) {
	...
	repo := &settingHandlerRepoStub{
		values: map[string]string{
			...
			service.SettingKeyAuthSourceDefaultEmailSubscriptions: `[{"value":31,"validity_days":15}]`,
...
func TestSettingHandler_UpdateSettings_PreservesOmittedAuthSourceDefaults(t *testing.T) {
	...
	require.Equal(t, `[{"value":31,"validity_days":15}]`, repo.values[service.SettingKeyAuthSourceDefaultEmailSubscriptions])
	...
	changed := diffSettings(
		...
		&service.AuthSourceDefaultSettings{
			Email: service.ProviderDefaultGrantSettings{
				Balance:          12.5,
				Concurrency:      7,
				Subscriptions:    []service.DefaultSubscriptionSetting{{Value: 21, ValidityDays: 30}},
				GrantOnSignup:    false,
