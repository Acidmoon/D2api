...
func (s *defaultOpenAIAccountScheduler) trySelectByLoadBalancePool(
	...
	if len(attempt.selectionOrder) == 0 {
		...
		s.service.notifyGroupUnavailable(ctx, req, nil, "load_plan", len(filtered), attempt.candidateCount, attempt.topK, "no candidate account remained after load planning")
		return attempt
	}
...
func (s *defaultOpenAIAccountScheduler) finishLoadBalanceSelectionFallback(
	...
	s.service.notifyGroupUnavailable(ctx, req, nil, "acquire", len(attempt.selectionOrder), candidateCount, topK, "all candidate accounts failed concurrency acquisition or final database recheck")
	return nil, candidateCount, topK, loadSkew, noAvailableOpenAISelectionError(req.RequestedModel, compactBlocked, filterStats.summary("selection_order_exhausted"))
}

// notifyGroupUnavailable sends a cooldown-protected alert only for groups that
// explicitly opted into unavailable-account notifications.
