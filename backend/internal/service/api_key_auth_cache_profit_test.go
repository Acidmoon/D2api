...
func TestAPIKeyAuthSnapshotProfitControlRoundtrip(t *testing.T) {
	...
	require.Equal(t, apiKeyAuthSnapshotVersion, snapshot.Version, "认证快照版本必须与当前常量一致（v24 起白名单字段随快照下发）")
...
func TestAPIKeyAuthSnapshotGroupModelPricingRoundtrip(t *testing.T) {
	...
	require.InDelta(t, 3.0, *materialized.Group.ModelPricing[0].InputPrice, 1e-12)
	require.NotNil(t, materialized.Group.ModelPricing[0].OutputPrice)
	require.InDelta(t, 15.0, *materialized.Group.ModelPricing[0].OutputPrice, 1e-12)
}
