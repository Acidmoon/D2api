...
// 注意：上游的 TestSettingService_UpdateSettings_DefaultSubscriptions_ValidGroup 依赖
// DefaultSubscriptionSetting.GroupID；本 fork 已将默认订阅改为钱包额度(Value)模型，
// GroupID 字段不存在，故不合并该用例，保留本 fork 的 PersistsBalanceValues 用例。
