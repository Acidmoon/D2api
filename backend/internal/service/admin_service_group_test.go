...
// 注意：上游另有 TestAdminService_UpdateGroup_ClearsPeakRateWhenChangingToStandard，
// 其断言"切回 standard 时清空高峰配置"；本 fork 的 NormalizePeakRateConfig 刻意忽略
// subscription_type（standard 分组也保留高峰配置），两者语义互斥，故不合并该用例，
// 保留本 fork 的 PreservesPeakRateWhenNormalizingLegacyGroup 行为用例。
func TestAdminService_UpdateGroup_PreservesPeakRateWhenNormalizingLegacyGroup(t *testing.T) {
	existingGroup := &Group{
		ID:                 1,
		Name:               "existing-group",
		Platform:           PlatformOpenAI,
...
func TestAdminService_CreateGroup_InvalidRequestFallbackIgnoresLegacyBillingType(t *testing.T) {
	fallbackID := int64(10)
	repo := &groupRepoStubForInvalidRequestFallback{
		groups: map[int64]*Group{
			fallbackID: {ID: fallbackID, Platform: PlatformAnthropic, SubscriptionType: SubscriptionTypeStandard},
