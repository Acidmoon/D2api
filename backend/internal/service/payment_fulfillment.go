...
// subscriptionFulfillmentPlanSnapshot freezes the limits purchased by the
// user so later plan edits cannot retroactively change an existing wallet.
func subscriptionFulfillmentPlanSnapshot(ctx context.Context, configService *PaymentConfigService, o *dbent.PaymentOrder) (string, *float64, *float64, *float64) {
	...
	plan, err := configService.GetPlan(ctx, *o.PlanID)
	if err != nil || plan == nil {
		return planName, dailyLimit, weeklyLimit, monthlyLimit
	}
