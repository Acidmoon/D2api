...
type Group struct {
	...
	// SubscriptionType is retained for database/API compatibility. It is always
	// normalized to standard and must not affect routing or billing decisions.
...
// 所有标准路由分组均可启用高峰倍率，并要求 start/end 合法且 end>start（不支持跨天），multiplier>=0。
...
// 关闭高峰时保留已配置的合法窗口（便于临时停用后再启用），但清掉无法解析的脏字符串与负倍率，避免脏数据入库。
//
// 与 ValidatePeakRateConfig 的分工：enabled=true 时校验已保证各字段合法，本函数为无操作；
// enabled=false 时校验放行，由本函数兜底清洗。调用顺序为先归一化、后校验，
func NormalizePeakRateConfig(_ string, enabled bool, start, end string, multiplier float64) (bool, string, string, float64) {
	if !enabled {
