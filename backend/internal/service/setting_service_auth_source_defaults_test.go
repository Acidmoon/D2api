...
func TestSettingService_GetAuthSourceDefaultSettings_ParsesValuesAndDefaults(t *testing.T) {
	...
	require.Equal(t, []DefaultSubscriptionSetting{{Value: 11, ValidityDays: 30}}, got.Email.Subscriptions)
...
func TestSettingService_UpdateAuthSourceDefaultSettings_PersistsAllKeys(t *testing.T) {
	...
	err := svc.UpdateAuthSourceDefaultSettings(context.Background(), &AuthSourceDefaultSettings{
		Email: ProviderDefaultGrantSettings{
			Balance:          1.25,
			Concurrency:      3,
			Subscriptions:    []DefaultSubscriptionSetting{{Value: 21, ValidityDays: 14}},
			GrantOnSignup:    false,
		...
		LinuxDo: ProviderDefaultGrantSettings{
			Balance:          2,
			Concurrency:      4,
			Subscriptions:    []DefaultSubscriptionSetting{{Value: 22, ValidityDays: 30}},
			GrantOnSignup:    true,
		...
		OIDC: ProviderDefaultGrantSettings{
			Balance:          3,
			Concurrency:      5,
			Subscriptions:    []DefaultSubscriptionSetting{{Value: 23, ValidityDays: 60}},
			GrantOnSignup:    true,
			GrantOnFirstBind: true,
		},
		WeChat: ProviderDefaultGrantSettings{
			Balance:          4,
			Concurrency:      6,
			Subscriptions:    []DefaultSubscriptionSetting{{Value: 24, ValidityDays: 90}},
			GrantOnSignup:    false,
			GrantOnFirstBind: false,
		},
	...
	require.Equal(t, []DefaultSubscriptionSetting{{Value: 24, ValidityDays: 90}}, got)
}
