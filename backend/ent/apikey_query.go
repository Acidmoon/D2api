...
func (_q *APIKeyQuery) QueryPrimaryGroup() *GroupQuery {
	query := (&GroupClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
...
func (_q *APIKeyQuery) WithPrimaryGroup(opts ...func(*GroupQuery)) *APIKeyQuery {
	query := (&GroupClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *APIKeyQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*APIKey, error) {
	...
	if query := _q.withPrimaryGroup; query != nil {
		if err := _q.loadPrimaryGroup(ctx, query, nodes, nil,
			func(n *APIKey, e *Group) { n.Edges.PrimaryGroup = e }); err != nil {
			return nil, err
		}
	}
...
func (_q *APIKeyQuery) loadPrimaryGroup(ctx context.Context, query *GroupQuery, nodes []*APIKey, init func(*APIKey), assign func(*APIKey, *Group)) error {
	ids := make([]int64, 0, len(nodes))
	nodeids := make(map[int64][]*APIKey)
	for i := range nodes {
...
func (_q *APIKeyQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := _q.ctx.Fields; len(fields) > 0 {
		...
		if _q.withPrimaryGroup != nil {
			_spec.Node.AddColumnOnce(apikey.FieldPrimaryGroupID)
		}
	}
