...
type Group struct {
	...
	// 是否在该分组所有账号不可用时发送邮件警告
	UnavailableAlertEnabled bool `json:"unavailable_alert_enabled,omitempty"`
	// 余额充值倍率覆盖，NULL 表示回落到全局设置
...
type GroupEdges struct {
	...
	// PrimaryAPIKeys holds the value of the primary_api_keys edge.
	PrimaryAPIKeys []*APIKey `json:"primary_api_keys,omitempty"`
...
// PrimaryAPIKeysOrErr returns the PrimaryAPIKeys value or an error if the edge
// was not loaded in eager-loading.
func (e GroupEdges) PrimaryAPIKeysOrErr() ([]*APIKey, error) {
	if e.loadedTypes[1] {
		return e.PrimaryAPIKeys, nil
	}
	return nil, &NotLoadedError{edge: "primary_api_keys"}
}
...
func (_m *Group) assignValues(columns []string, values []any) error {
	...
	for i := range columns {
		...
		case group.FieldUnavailableAlertEnabled:
			if value, ok := values[i].(*sql.NullBool); !ok {
				return fmt.Errorf("unexpected type %T for field unavailable_alert_enabled", values[i])
			} else if value.Valid {
				_m.UnavailableAlertEnabled = value.Bool
			}
		case group.FieldRechargeMultiplier:
			if value, ok := values[i].(*sql.NullFloat64); !ok {
				return fmt.Errorf("unexpected type %T for field recharge_multiplier", values[i])
			} else if value.Valid {
				_m.RechargeMultiplier = new(float64)
				*_m.RechargeMultiplier = value.Float64
			}
...
// QueryPrimaryAPIKeys queries the "primary_api_keys" edge of the Group entity.
func (_m *Group) QueryPrimaryAPIKeys() *APIKeyQuery {
	return NewGroupClient(_m.config).QueryPrimaryAPIKeys(_m)
}
...
func (_m *Group) String() string {
	...
	builder.WriteString(", ")
	builder.WriteString("unavailable_alert_enabled=")
	builder.WriteString(fmt.Sprintf("%v", _m.UnavailableAlertEnabled))
	builder.WriteString(", ")
	if v := _m.RechargeMultiplier; v != nil {
		builder.WriteString("recharge_multiplier=")
		builder.WriteString(fmt.Sprintf("%v", *v))
	}
	builder.WriteString(", ")
