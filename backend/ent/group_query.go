...
type GroupQuery struct {
	...
	withPrimaryAPIKeys    *APIKeyQuery
...
// QueryPrimaryAPIKeys chains the current query on the "primary_api_keys" edge.
func (_q *GroupQuery) QueryPrimaryAPIKeys() *APIKeyQuery {
	query := (&APIKeyClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
...
func (_q *GroupQuery) Clone() *GroupQuery {
	...
	return &GroupQuery{
		...
		withPrimaryAPIKeys:    _q.withPrimaryAPIKeys.Clone(),
...
// WithPrimaryAPIKeys tells the query-builder to eager-load the nodes that are connected to
// the "primary_api_keys" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *GroupQuery) WithPrimaryAPIKeys(opts ...func(*APIKeyQuery)) *GroupQuery {
	query := (&APIKeyClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withPrimaryAPIKeys = query
	return _q
}
...
func (_q *GroupQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Group, error) {
	...
	if query := _q.withPrimaryAPIKeys; query != nil {
		if err := _q.loadPrimaryAPIKeys(ctx, query, nodes,
			func(n *Group) { n.Edges.PrimaryAPIKeys = []*APIKey{} },
			func(n *Group, e *APIKey) { n.Edges.PrimaryAPIKeys = append(n.Edges.PrimaryAPIKeys, e) }); err != nil {
			return nil, err
		}
	}
...
func (_q *GroupQuery) loadPrimaryAPIKeys(ctx context.Context, query *APIKeyQuery, nodes []*Group, init func(*Group), assign func(*Group, *APIKey)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int64]*Group)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(apikey.FieldPrimaryGroupID)
	}
	query.Where(predicate.APIKey(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(group.PrimaryAPIKeysColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.PrimaryGroupID
		if fk == nil {
			return fmt.Errorf(`foreign-key "primary_group_id" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "primary_group_id" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
